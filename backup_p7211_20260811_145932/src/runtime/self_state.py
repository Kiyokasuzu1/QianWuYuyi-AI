...
class SelfState:
    ...
    mood: str = "平静"
    curiosity: float = 0.5
    # social_need / initiative 默认值上调到 0.65，让 RuntimeCore.tick 的规则引擎
    # 在启动后 1~2 个周期内就能满足阈值（initiative_min=0.5 / social_need_min=0.6），
    # 否则首次触发要等 4+ 小时甚至永远达不到。
    social_need: float = 0.65
    focus: float = 0.6
    trust: float = 0.5
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "SelfState":
        ...
        return cls(
            ...
            # 兼容旧版较低的 social_need / initiative，首次加载后新值就会生效
            social_need=data.get("social_need", 0.65),
            focus=data.get("focus", 0.6),
            trust=data.get("trust", 0.5),
            initiative=data.get("initiative", 0.65),
