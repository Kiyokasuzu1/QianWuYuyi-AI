...
STATUS: FUTURE / SNAPSHOT INFRASTRUCTURE
  本模块是未来 Runtime SelfModel 快照持久化层，并非当前 Production SelfModel Authority。
  当前 Authority 为: src/personality/self_model_store.py

  本模块依赖 SelfModelSnapshot / identity_id / runtime persistence 体系，
  与 personality/self_model_store.py 是两套独立数据模型。
  未来 P4 架构迁移时，两个系统可能正式合并。
