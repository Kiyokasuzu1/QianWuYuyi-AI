...
class RuntimePipeline:
    ...
    def run(self, input_data: Any) -> "RuntimeContext":
        ...
        # 1) 解析 user_message + user_id（Phase 7.2.1.1：identity stabilization）
        user_message = self._extract_user_message(input_data)
        context_user_id: Optional[str] = self._extract_user_id(input_data)
        ...
        try:
            context = RuntimeContext(
                ...
                inputs={
                    ...
                    # Phase 7.2.1.1：把入口 user_id 写入 context inputs
                    # （不修改 RuntimeContext schema，只保存在 inputs 字典里，下游可读取）
        ...
        try:
            ...
            if not reply:
                try:
                    # Phase 7.2.1.1：入口 user_id 必须透传给 Orchestrator，
                    # 防止 Orchestrator 内部的 UserResolver 乱猜身份（已经在 Orchestrator 侧
                    # 加了保护：只要 user_id 参数不为 None，就直接用，不再 fallback resolver）
    ...
    # ================================================================
    # Phase 7.2.1.1-identity-stabilization: 提取 user_id（不修改 schema）
    # ================================================================
    def _extract_user_id(self, input_data: Any) -> Optional[str]:
        """从 input_data 提取 user_id。只做提取，不重新解析、不 fallback。

        设计原则：RuntimePipeline 不是身份决定者，只负责把入口传的 user_id
        透传到 context 和下游 orchestrator fallback。
        """
        if isinstance(input_data, str):
            return None
        if isinstance(input_data, dict):
            # 按优先级顺序查找：user_id > user > uid
