...
function switchView(view) {
  ...
  if (view === 'dashboard' || view === 'modules') {
    ...
    showCognitiveView();
  } else if (view === 'selfmodel') {
    showSelfModelView();
  } else if (view === 'selfmodel-timeline') {
    showSelfModelTimelineView();
  } else if (view === 'selfmodel-health') {
    showSelfModelHealthView();
...
function showDashboardView() {
  ...
  document.querySelectorAll('.config-view, .audit-view, .cognitive-view, .autonomous-view, .remote-view, .agents-view, .selfmodel-view, .smtl-view, .smhealth-view').forEach(el => {
...
function showCognitiveView() {
  ...
  if (typeof YuyiReasoning !== 'undefined') {
    ...
    if (YuyiReasoning.init(reasoningContainerId)) {
      YuyiReasoning.load('emotion', false); // 默认加载情绪状态解释（真实数据）
    }
  }
  // 初始化认知雷达（Phase 2.5）
  ...
  if (typeof YuyiMemoryGraph !== 'undefined') {
    ...
    if (YuyiMemoryGraph.init(canvasId)) {
      YuyiMemoryGraph.load(false); // 默认使用真实数据
    }
  }
  ...
  if (typeof YuyiRelationshipTimeline !== 'undefined') {
    ...
    if (YuyiRelationshipTimeline.init(timelineContainerId)) {
      YuyiRelationshipTimeline.load(false); // 默认使用真实数据
    }
  }
}
...
function hideDashboardView() {
  ...
  document.querySelectorAll('.cognitive-view, .remote-view, .agents-view, .selfmodel-view, .smtl-view, .smhealth-view').forEach(el => {
    el.style.display = 'none';
    el.classList.remove('active');
  });
}
...
function showRemoteView() {
  hideDashboardView();
  // 隐藏其他动态视图
  document.querySelectorAll('.config-view, .audit-view, .cognitive-view, .autonomous-view, .agents-view, .selfmodel-view, .smtl-view, .smhealth-view').forEach(el => {
    el.style.display = 'none';
    el.classList.remove('active');
  });
  const view = document.getElementById('remote-section');
  if (view) {
    view.style.display = '';
    view.classList.add('active');
  }
  // 初始化并加载数据
  if (typeof YuyiRemote !== 'undefined') {
    YuyiRemote.init();
    YuyiRemote.load();
  }
}
...
function showAgentsView() {
  hideDashboardView();
  // 隐藏其他动态视图
  document.querySelectorAll('.config-view, .audit-view, .cognitive-view, .autonomous-view, .remote-view, .selfmodel-view, .smtl-view, .smhealth-view').forEach(el => {
    el.style.display = 'none';
    el.classList.remove('active');
  });
  ...
  // 初始化并加载数据
  if (typeof YuyiAgents !== 'undefined') {
    YuyiAgents.init();
    YuyiAgents.load();
    // 默认开启自动刷新
    YuyiAgents.startAutoRefresh();
    // 同步复选框状态（避免重复切换时状态错乱）
    const checkbox = document.getElementById('agents-auto-refresh');
...
function showSelfModelView() {
  hideDashboardView();
  // 隐藏其他动态视图
  document.querySelectorAll('.config-view, .audit-view, .cognitive-view, .autonomous-view, .remote-view, .agents-view, .smtl-view, .smhealth-view').forEach(el => {
    el.style.display = 'none';
    el.classList.remove('active');
  });
  const view = document.getElementById('selfmodel-section');
  if (view) {
    view.style.display = '';
    view.classList.add('active');
  }
  // 触发 SelfModel Dashboard 加载
  if (window.YuyiSelfModelDashboard && typeof YuyiSelfModelDashboard.render === 'function') {
    YuyiSelfModelDashboard.render(true);
  }
}

/* ============ 演化时间线视图（Phase 6.5 Step 5.3） ============ */

function showSelfModelTimelineView() {
  hideDashboardView();
  // 隐藏其他动态视图
  document.querySelectorAll('.config-view, .audit-view, .cognitive-view, .autonomous-view, .remote-view, .agents-view, .selfmodel-view, .smhealth-view').forEach(el => {
    el.style.display = 'none';
    el.classList.remove('active');
  });
  const view = document.getElementById('selfmodel-timeline-section');
  if (view) {
    view.style.display = '';
    view.classList.add('active');
  }
  // 触发 Evolution Timeline 加载
  if (window.YuyiSelfModelTimeline && typeof YuyiSelfModelTimeline.load === 'function') {
    YuyiSelfModelTimeline.load(true);
  }
  // 绑定刷新按钮
  const refreshBtn = document.getElementById('smtl-refresh-btn');
  if (refreshBtn && !refreshBtn._smtlBound) {
    refreshBtn.addEventListener('click', function () {
      if (window.YuyiSelfModelTimeline && typeof YuyiSelfModelTimeline.load === 'function') {
        YuyiSelfModelTimeline.load(true);
      }
    });
    refreshBtn._smtlBound = true;
  }
  // 更新 last-updated 时间
  const updated = document.getElementById('smtl-last-updated');
  if (updated) updated.textContent = new Date().toLocaleTimeString();
}

/* ============ Health + Retention 视图（Phase 6.5 Step 5.4） ============ */

function showSelfModelHealthView() {
  hideDashboardView();
  // 隐藏其他动态视图
  document.querySelectorAll('.config-view, .audit-view, .cognitive-view, .autonomous-view, .remote-view, .agents-view, .selfmodel-view, .smtl-view').forEach(el => {
    el.style.display = 'none';
    el.classList.remove('active');
  });
  const view = document.getElementById('selfmodel-health-section');
  if (view) {
    view.style.display = '';
    view.classList.add('active');
  }
  // 触发 Health / Retention 加载
  if (window.YuyiSelfModelHealth && typeof YuyiSelfModelHealth.load === 'function') {
    YuyiSelfModelHealth.load(true);
  }
  // 绑定刷新按钮
  const refreshBtn = document.getElementById('smh-refresh-btn');
  if (refreshBtn && !refreshBtn._smhBound) {
    refreshBtn.addEventListener('click', function () {
      if (window.YuyiSelfModelHealth && typeof YuyiSelfModelHealth.load === 'function') {
        YuyiSelfModelHealth.load(true);
      }
    });
    refreshBtn._smhBound = true;
  }
  // 更新 last-updated 时间
  const updated = document.getElementById('smh-last-updated');
  if (updated) updated.textContent = new Date().toLocaleTimeString();
}
