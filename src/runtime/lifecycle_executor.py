...
class LifecycleExecutor:
    ...
    def execute(
        ...
        """
        # 0) 初始化 ctx 安全网
        # Phase 4.0.4-Pre: 先归一化 ctx 类型——外部若传入 frozen lifecycle_context，
        # 会导致后续 ctx.xxx = yyy 全部失败，必须在这里强制转成 mutable。
        foreign = (ctx is not None and not isinstance(ctx, RuntimeContext))
        ctx = _normalize_mutable_ctx(ctx)
        if foreign:
            logger.info(
                "[LifecycleExecutor] 归一化外部 ctx: original_type=%s.%s session_id=%s",
        ...
        if not hasattr(ctx, "_phase_errors") or not isinstance(
            ...
            try:
                ...
                # Phase 4.0.4-Pre: 如果这里还失败，ctx 还是 immutable，
                # 立刻重新全新构造一个 mutable ctx，保证 Stage 不会全部隔离。
                logger.warning(
                    "[LifecycleExecutor] ctx._phase_errors 写失败，"
                    "强制重建全新 mutable RuntimeContext: err=%s", exc_p,
                )
                ctx = RuntimeContext()
                ctx._phase_errors = {}  # type: ignore[attr-defined]
