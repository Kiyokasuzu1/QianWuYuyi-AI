        ...
        if self._enable_store and self._store is not None:
            try:
                if hasattr(self._store, "load"):
                    ...
                    all_memories = []
                # Phase 2.5-B: scope 授权过滤(替代 Phase 4.1 的精确 user_id 匹配)。
                # owner 私人层 + YUI_CORE + global 关系核心均按 memory_scope 授权。
                if effective_uid:
                    try:
                        from src.memory.memory_scope import resolve_allowed
                        all_memories = [
                            m for m in all_memories
                            if isinstance(m, dict) and resolve_allowed(m, effective_uid)
                        ]
                    except Exception:  # noqa: BLE001
                        # 回退旧口径:精确 user_id 匹配(向后兼容)
                        all_memories = [
                            m for m in all_memories
