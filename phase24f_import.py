# -*- coding: utf-8 -*-
"""Phase 2.4-F:T2c A类受控导入脚本。

与 phase24a_import.py 同源,安全标准一致:
- 只通过现有 MemoryStore.add() 写入流程,不直接替换 memory.json
- 每条记录保留 metadata.recovery(原 provenance 完整保留,新增 batch_id=phase24f-t2c / source=historical_recovery)
- 不删除任何已有记录,不修改 identity/growth/emotion 任何文件
- 导入前在内存中先做 PollutionGuard 预检 + 内容指纹去重
- 全程只 import src.memory.*

用法:
  python phase24f_import.py --payload phase24f/t2c_payload.jsonl \
      --memory-path data/memory.json --report phase24f/import_report.json
  python phase24f_import.py --rollback phase24f/backup/memory.pre_t2c_import.json \
...
def build_memory(rec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    md["source"] = "recovery_import_phase24f"
...
def main() -> int:
    ...
    ap.add_argument("--payload", type=Path, default=Path("phase24f/t2c_payload.jsonl"))
    ap.add_argument("--memory-path", type=Path, default=Path("data/memory.json"))
    ap.add_argument("--report", type=Path, default=Path("phase24f/import_report.json"))
