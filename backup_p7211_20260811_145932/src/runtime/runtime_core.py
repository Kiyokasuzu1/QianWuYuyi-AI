...
class RuntimeCore(ModuleBase):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # DecisionEngine 配置：兼容 config["decision"] + config["decision_engine"]
        # （config.yaml 新写 decision_engine，老版本仍保留 decision key）
        _de_cfg: Dict[str, Any] = {}
        _de_cfg.update(self.config.get("decision", {}) or {})
        _de_cfg.update(self.config.get("decision_engine", {}) or {})
        self.decision_engine = DecisionEngine(config=_de_cfg)
        ...
        # Phase 7.2.1-p3：长期独处回升计时器。
        # 记录最近一次 user_input / proactive_executed 事件时间，
        # 超过 LONELINESS_THRESHOLD_SECONDS 后，每分钟 tick 会微量回升
        # social_need / initiative，避免长时间不说话后永远触发不了主动消息。
        self._last_interaction_at: float = time.time()
        self._loneliness_threshold_seconds: float = float(
            self.config.get("loneliness_threshold_seconds", 600.0)  # 默认 10 分钟
        )
        # 每分钟的回升幅度（每小时 ~ 0.24 / 每小时 ~0.18，约 2.5 小时从 0.00 回到 0.60）
        self._loneliness_gain_per_minute: Dict[str, float] = {
            "social_need": float(self.config.get("loneliness_social_gain_per_min", 0.004) or 0.004),
            "initiative": float(self.config.get("loneliness_initiative_gain_per_min", 0.003) or 0.003),
        }
    ...
    def tick(self) -> None:
        ...
        # Phase 7.2.1-p3：长期独处回升（超过 loneliness_threshold_seconds 没有用户消息/主动行动）
        # 每次 tick 按"每分钟"比例增加 social_need / initiative。
        loneliness_delta = now - float(self._last_interaction_at or now)
        loneliness_triggered: bool = False
        loneliness_gain: Dict[str, float] = {}
        if loneliness_delta >= self._loneliness_threshold_seconds:
            loneliness_triggered = True
            minutes = max(1.0, float(delta) / 60.0)
            social_gain = float(self._loneliness_gain_per_minute.get("social_need", 0.0) or 0.0)
            init_gain = float(self._loneliness_gain_per_minute.get("initiative", 0.0) or 0.0)
            if social_gain > 0:
                before = float(self.self_state.social_need)
                self.self_state.social_need = min(1.0, before + social_gain * minutes)
                loneliness_gain["social_need"] = round(
                    float(self.self_state.social_need) - before, 4
                )
            if init_gain > 0:
                before = float(self.self_state.initiative)
                self.self_state.initiative = min(1.0, before + init_gain * minutes)
                loneliness_gain["initiative"] = round(
                    float(self.self_state.initiative) - before, 4
                )
        ...
        # 触发决策（_maybe_decide 内部会打印 INFO 级摘要）
        dispatched = self._maybe_decide()
        ...
        # Phase 7.2.1-p2：tick INFO 日志，便于用户从 logs/yuyi-api.log 直接观测生命循环
        # 注意：只在首次/每 10 次/有 dispatched/保存状态/独处回升触发 时打印，避免 INFO 日志刷屏
        should_log = (
            self._tick_count <= 2
            or self._tick_count % 10 == 0
            or bool(dispatched)
            or saved
            or (loneliness_triggered and bool(loneliness_gain))
        )
        if should_log:
            loneliness_part = ""
            if loneliness_triggered:
                loneliness_part = (
                    " | idle={:.0f}min loneliness_gain={}".format(
                        loneliness_delta / 60.0,
                        " ".join(f"{k}={v:+.3f}" for k, v in loneliness_gain.items()) or "none",
                    )
                )
            logger.info(
                "RuntimeCore.tick heartbeat | tick=%d | delta=%.1fs | "
                "initiative=%.2f social_need=%.2f energy=%.2f | "
                "dispatched=%d saved=%s%s",
                self._tick_count, delta,
                self.self_state.initiative, self.self_state.social_need, self.self_state.energy,
                len(dispatched), saved,
                loneliness_part,
            )

    # ==================== 决策 ====================

    def _maybe_decide(self) -> List[Action]:
        """
        判断是否产生决策
        ...
        Phase 7.2.1-p2: 返回 List[Action]，便于日志/测试观测。
        """
        # 层 1：规则引擎
        ...
        # Phase 7.2.1-p2：产生了 send_message 主动消息决策时，打印 INFO 摘要，
        # 便于用户在 logs/yuyi-api.log 中直接看到触发原因
        send_actions = [a for a in dispatched_actions if a.action_type == "send_message"]

        # Phase 7.2.1-p3：ActionDispatcher.dispatch 若没注册 handler（status == "no_handler"），
        # 会静默算入 dispatched，但实际没执行任何逻辑；必须打 WARNING。
        # 典型根因：InitiativeBridge.register() 使用的 RuntimeBridge 单例
        # ≠ api_server.py 里实际创建 RuntimeCore 的 RuntimeBridge，两者的 action_dispatcher 不共享。
        no_handler = [a for a in dispatched_actions if getattr(a, "status", None) == "no_handler"]
        if no_handler:
            missing_types = sorted({a.action_type for a in no_handler})
            try:
                registered = sorted(list(self.action_dispatcher._handlers.keys()))
            except Exception:
                registered = []
            logger.warning(
                "RuntimeCore._maybe_decide: %d 条 action 没有对应 handler（status=no_handler）：%s。"
                "请确认 InitiativeBridge / MemoryAdapter / PersonalityAdapter 等已通过同一 RuntimeBridge 实例注册。"
                " 当前 RuntimeCore action_dispatcher 已注册 handler: %s",
                len(no_handler), missing_types, registered,
            )
        if send_actions:
            ss = self.self_state
            sent_ok = sum(1 for a in send_actions
                          if (a.result or {}).get("sent") is True)
            skipped_cd = sum(1 for a in send_actions
                             if (a.result or {}).get("skipped_by_cooldown") is True)
            no_msg = sum(1 for a in send_actions
                         if (a.result or {}).get("reason") == "no message generated; skipped")
            no_tgt = sum(1 for a in send_actions
                         if (a.result or {}).get("reason") == "no target_user configured")
            send_fail = sum(1 for a in send_actions
                            if (a.result or {}).get("reason") == "send_message returned False")
            logger.info(
                "RuntimeCore._maybe_decide: 触发 send_message 主动决策 count=%d "
                "(sent_ok=%d cooldown=%d no_message=%d no_target=%d send_fail=%d no_handler=%d) | "
                "initiative=%.2f social_need=%.2f | reason=%s",
                len(send_actions), sent_ok, skipped_cd, no_msg, no_tgt, send_fail,
                len([a for a in send_actions if getattr(a, "status", None) == "no_handler"]),
                ss.initiative, ss.social_need,
                send_actions[0].reason,
            )
        elif self._tick_count <= 3 or self._tick_count % 20 == 0:
            # 前几个 tick + 每 20 个 tick 打一条 "no decision" 观测日志，
            # 方便判断是规则未命中还是代码没执行
            ss = self.self_state
            logger.info(
                "RuntimeCore._maybe_decide: 本周期无决策 | "
                "initiative=%.2f(min=%.2f) social_need=%.2f(min=%.2f) | "
                "decisions_considered=%d",
                ss.initiative, self.decision_engine.thresholds["initiative_min"],
                ss.social_need, self.decision_engine.thresholds["social_need_min"],
                len(decisions),
            )
    ...
    def _load_state(self) -> None:
        ...
        Phase 7.2.1-p3：历史 state 文件可能存在 initiative/social_need 极低值（比如 0.00/0.27），
        来自旧版 SelfState 默认值 + 长期用户消息 social_need-0.1 累积；
        如果直接使用会导致重启后几个小时内都触发不了主动消息。
        这里做最低值兜底：任一项低于决策阈值 - 0.05 的安全余量，就抬到合理基线。
        """
        ...
        try:
            ...
            # Phase 7.2.1-p3：主动消息最低触发基线（比默认阈值再留 0.05 余量）。
            # 注意：仅当历史值低于基线时才抬到基线；高于基线的保留原值不动（避免覆盖高状态）。
            th_i = float(self.decision_engine.thresholds.get("initiative_min", 0.55) or 0.55)
            th_s = float(self.decision_engine.thresholds.get("social_need_min", 0.55) or 0.55)
            base_i = max(th_i, th_i + 0.05)  # ≈ 0.60
            base_s = max(th_s, th_s + 0.05)  # ≈ 0.60
            raised: List[str] = []
            if float(self.self_state.initiative) < base_i:
                before = float(self.self_state.initiative)
                self.self_state.initiative = base_i
                raised.append(f"initiative {before:.2f}→{base_i:.2f}")
            if float(self.self_state.social_need) < base_s:
                before = float(self.self_state.social_need)
                self.self_state.social_need = base_s
                raised.append(f"social_need {before:.2f}→{base_s:.2f}")
            if raised:
                self.world_state.self_state = self.self_state
                logger.info(
                    "RuntimeCore 状态恢复基线已抬高（%s）；避免重启后主动消息几小时内无法触发",
                    " ; ".join(raised),
                )
            else:
    ...
    def process(
        ...
        with self._process_lock:
            ...
            # Phase 7.2.1-p3：交互事件（用户消息 / 主动行动）刷新"最近交互时间"，
            # 独处计时器 tick 里据此决定是否开始回升 social_need / initiative。
            if event_type in ("user_input", "message_received", "action.proactive_executed"):
    ...
    def _stage_02_memory_retrieval(
        ...
        if getattr(ctx, "_control_blocked", False):
            return
        # 优先 path: memory_adapter.retrieve(ctx)（若存在）
        ...
        # 降级 path: 直接读 memory_store.get_recent（若有 memory_store 引用）
        ms = None
        ...
        if ms is not None and callable(getattr(ms, "get_recent", None)):
            try:
                ctx.retrieved_memories = ms.get_recent(limit=20)  # type: ignore[attr-defined]
            except Exception as exc:
                logger.warning(
                    "[RuntimeCore.process] memory_store.get_recent 失败: %s", exc,
                )
    ...
    def _stage_04_growth_evaluation(
        ...
        if getattr(ctx, "_control_blocked", False):
            return
        # SAFE 模式: Growth 自动更新被禁止
        ga = getattr(self, "growth_adapter", None)
        if ga is None:
            return
        ...
        try:
            ...
            logger.warning(
                "[RuntimeCore.process] growth_evaluation 失败: %s", exc,
            )
    ...
    def _stage_09_self_model_build(
        ...
        """阶段 9: Self Model Build（默认 no-op，Phase 4.2.1 SelfModel 接入）。"""
        return

    def _stage_10_self_model_evolution(
        self, event: Optional[Event], ctx: RuntimeContext,
    ) -> None:
        """阶段 10: Self Model Evolution（默认 no-op，Phase 4.5 真实激活）。"""
        return

    def _stage_11_self_model_reflection(
        self, event: Optional[Event], ctx: RuntimeContext,
    ) -> None:
        """阶段 11: Self Model Reflection（默认 no-op，Phase 4.7 真实激活）。"""
        return

    def _stage_12_self_model_validation(
        self, event: Optional[Event], ctx: RuntimeContext,
    ) -> None:
        """阶段 12: Self Model Validation（默认 no-op，Phase 4.7 真实激活）。"""
        return

    def _stage_13_self_model_persistence(
        self, event: Optional[Event], ctx: RuntimeContext,
    ) -> None:
        """阶段 13: Self Model Persistence（默认 no-op，Phase 4.6 真实激活）。"""
        return
