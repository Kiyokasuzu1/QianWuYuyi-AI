...
# Phase 4.2-A 契约统一：经 Gate（用户）批准，扩展 4 个提取管线类型，
# 使 TypedDict 成为新旧两个生产者共用的唯一事实来源。
RelationshipEventType = Literal[
    ...
    # ---- Phase 4.2-A：Extractor 提取管线类型（源自用户消息的关键词模式） ----
    "collaboration",        # 协作（一起/共同/合作/项目/长期...）
    "trust_building",       # 信任建设（相信/信任/放心/可靠...）
    "boundary_respect",     # 边界尊重（尊重/理解/不勉强...）
    "preference_learning",  # 偏好学习（习惯/偏好/了解...）
]
...
class RelationshipEvent(TypedDict, total=False):
    ...
    user_id: str
    participants: list
    # ---- Phase 4.2-A 提取管线扩展键（Extractor 产出时填写）----
    evidence_ids: list          # 关联证据 memory id 列表（泛化 source_memory_id）
    potential_dimensions: list  # 提取期声明的潜在影响维度（观察级假设）
...
# Phase 4.2-A：提取管线扩展键（不属于 bridge 事件的 exact 契约，
# 但属于统一 schema 的已登记字段——Store 不视为「未登记字段」）
RELATIONSHIP_EVENT_EXTRACTION_KEYS: tuple = (
    "evidence_ids",
    "potential_dimensions",
)

# 统一 schema 的全部已登记字段（Store 的 extra-key 判定集）
RELATIONSHIP_EVENT_REGISTERED_KEYS: tuple = (
    RELATIONSHIP_EVENT_FROZEN_KEYS + RELATIONSHIP_EVENT_EXTRACTION_KEYS
)

RELATIONSHIP_EVENT_ALLOWED_TYPES: tuple = (
    "promise", "declaration", "milestone", "support",
    "boundary", "shared_activity", "other",
    # Phase 4.2-A：提取管线类型（与 RelationshipEventType 扩展一致）
    "collaboration", "trust_building", "boundary_respect", "preference_learning",
)
