...
v1.2 (Phase 3.8.2-A):
- 新增 experience_meaning 参数：接入 ExperienceInterpreter 的语义理解结果
- 当提供 experience_meaning 时，target_candidates 优先使用 suggested_growth_directions
- 低置信度或高风险 meaning 不会绕过正常评估流程
- 保持向后兼容：不传 experience_meaning 时走旧逻辑
...
class GrowthEvaluator:
    ...
    def evaluate(
        ...
        experience_meaning: Optional[Dict] = None,
    ) -> GrowthEvent:
        """
        评估一个事件是否值得进入成长系统。
        ...
        Args:
            ...
            experience_meaning: Phase 3.8.2-A 新增。
                ExperienceMeaning.to_dict() 的结果，来自 deep_resolve_meaning()。
                为 None 时走旧逻辑（GROWTH_SIGNAL_CANDIDATES 规则映射）。
        ...
        # 10. 目标维度候选
        # Phase 3.8.2-A：优先使用 ExperienceMeaning 的语义理解结果
        meaning_used = False
        if experience_meaning and isinstance(experience_meaning, dict):
            meaning_used = self._apply_experience_meaning(
                event, experience_meaning
            )
        ...
        if not meaning_used:
            ...
            # Phase 3.8.2-A：语义理解成功，使用 meaning 提供的维度
            target_candidates = event.pop("_meaning_target_candidates", [])
            growth_signal = "experience_meaning"  # 标记来源
            # 高风险 meaning 不能绕过正常评估
            if event.pop("_meaning_has_critical_risk", False):
                growth_allowed = False
            # 低置信度 meaning 降低影响
            if event.pop("_meaning_low_confidence", False):
                impact = round(impact * 0.5, 3)
        ...
        return {
            **event,
            ...
            # Phase 3.8.2-A：附带经历意义（供下游消费）
            "experience_meaning": experience_meaning if meaning_used else None,
            # 语义理解元数据
            "_meaning_surface": event.pop("_meaning_surface", "") if meaning_used else "",
            "_meaning_deeper": event.pop("_meaning_deeper", "") if meaning_used else "",
    ...
    # ============================================================
    # Phase 3.8.2-A：经历意义接入
    # ============================================================
    def _apply_experience_meaning(
        self, event: Dict, experience_meaning: Dict
    ) -> bool:
        """将 ExperienceMeaning 的语义理解结果应用到当前评估。

        成功时设置 target_candidates 和相关标记，返回 True。
        失败（低置信度/高风险）时返回 False，调用方应回退到规则映射。

        Returns:
            True 表示 meaning 成功应用，False 表示需要 fallback
        """
        # 1. 检查置信度
        meaning_confidence = experience_meaning.get("confidence", 0.0)
        if meaning_confidence < 0.3:
            # 置信度过低，不信任语义理解，回退规则
            return False

        # 2. 检查风险标记
        risk_flags = experience_meaning.get("risk_flags", [])
        has_critical = any(
        ...
        if has_critical:
            # 有严重风险，标记但允许继续（让 Evaluator 的后续逻辑决定）
            event["_meaning_has_critical_risk"] = True

        # 3. 提取成长方向
        directions = experience_meaning.get("suggested_growth_directions", [])
        if not directions:
            return False

        # 4. 设置 target_candidates（从语义理解中提取维度名）
        dims = [
            d.get("dimension", "")
            for d in directions
        ...
        # 使用 nonlocal 变量 target_candidates（在 evaluate 中修改）
        # 由于 Python 不支持 nonlocal 跨方法，使用 event 字典传递
        event["_meaning_target_candidates"] = dims
        event["_meaning_used"] = True
        event["_meaning_surface"] = experience_meaning.get("surface_meaning", "")
        event["_meaning_deeper"] = experience_meaning.get("deeper_significance", "")

        # 5. 低置信度时降低成长幅度（但不阻止）
        if meaning_confidence < 0.5:
            event["_meaning_low_confidence"] = True
