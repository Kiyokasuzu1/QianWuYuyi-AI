...
class TestSelfModelUpdater(unittest.TestCase):
    """Phase 3.8.6: 测试新的 SelfModelUpdater + SelfModelUpdaterAdapter API"""

    def _make_growth_record(self) -> Dict[str, Any]:
        return create_growth_record(
            "gr_test_01", "e_test_01", "tone_pref_gentle",
            "preference", "context",
            {"warmth": 0.01, "initiative": 0.005},
            0.65, "测试成长记录",
    ...
    def _make_pcr(self) -> Dict[str, Any]:
        return {
            ...
            "growth_records": [
                create_growth_record(
                    "gr_pcr_01", "e_pcr", "high_user_activity_warmth",
                    "preference", "context",
                    {"warmth": 0.004}, 0.6, "PCR 附带 GR",
    ...
    def _make_insight(self) -> List[Dict[str, Any]]:
        return [
            {"insight_id": "ins_a", "pattern_detected": "high_frequency_proactive",
             "pattern_frequency": 5, "confidence": 0.7, "summary": "高频主动"},
            {"insight_id": "ins_b", "pattern_detected": "",
             "pattern_frequency": 0, "confidence": 0.5, "summary": "无 pattern"},
        ]

    # ============================================================
    # 1. create_proposal_from_growth（只生成，不写入）
    # ============================================================

    def test_01_create_proposal_from_growth(self):
        """创建 Proposal 不写入 Store"""
        store = SelfModelStore()
        updater = SelfModelUpdater(self_model_store=store)
        record = self._make_growth_record()

        # 快照前
        snapshot_before = store.get()
        proposal = updater.create_proposal_from_growth(record)
        snapshot_after = store.get()

        self.assertIsNotNone(proposal)
        self.assertIsInstance(proposal, SelfModelChangeProposal)
        self.assertEqual(proposal.change_type, "narrative_append")
        self.assertEqual(proposal.target, "growth_narratives")
        self.assertIn("narrative", proposal.change)
        self.assertEqual(proposal.source["growth_id"], "gr_test_01")
        # 硬契约：调用前后 Store 快照一致
        self.assertEqual(snapshot_before, snapshot_after)

    def test_02_update_from_growth_writes_to_store(self):
        """update_from_growth 立即写入 Store"""
        store = SelfModelStore()
        updater = SelfModelUpdater(self_model_store=store)
        record = self._make_growth_record()
    ...
    # ============================================================
    # 2. Adapter: from_pcr（只生成，不写入）
    # ============================================================

    def test_03_adapter_from_pcr_does_not_write(self):
        """Adapter.from_pcr() 不写入 Store"""
        ...
        snapshot_before = store.get()
        proposal = adapter.from_pcr(self._make_pcr())
        snapshot_after = store.get()

        self.assertIsNotNone(proposal)
        self.assertIsInstance(proposal, SelfModelChangeProposal)
        # 硬契约：from_pcr 不修改 Store
        self.assertEqual(snapshot_before, snapshot_after)

    # ============================================================
    # 3. Adapter: from_insights（只生成，不写入）
    # ============================================================

    def test_04_adapter_from_insights_does_not_write(self):
        """Adapter.from_insights() 不写入 Store"""
        from src.runtime.self_model.self_model_updater_adapter import SelfModelUpdaterAdapter
        ...
        snapshot_before = store.get()
        proposals = adapter.from_insights(self._make_insight())
        snapshot_after = store.get()

        # 至少 1 条 proposal（第一条有 pattern）
        self.assertGreaterEqual(len(proposals), 1)
        for p in proposals:
            self.assertIsInstance(p, SelfModelChangeProposal)
        # 硬契约：from_insights 不修改 Store
        self.assertEqual(snapshot_before, snapshot_after)

    # ============================================================
    # 4. Adapter: apply_suggestion（正式写入 Store）
    # ============================================================

    def test_05_apply_suggestion_writes_to_store(self):
        """Adapter.apply_suggestion() 写入 Store"""
        from src.runtime.self_model.self_model_updater_adapter import SelfModelUpdaterAdapter
        ...
        proposal = adapter.from_pcr(self._make_pcr())
        self.assertIsNotNone(proposal)
        proposal.requires_approval = False  # Phase 4.0.3-C: 模拟已审批

        snapshot_before = store.get()
        adapter.apply_suggestion(suggestion=proposal)
        snapshot_after = store.get()

        # 硬契约：apply_suggestion 后才写入 Store
        self.assertNotEqual(snapshot_before, snapshot_after)
    ...
    # ============================================================
    # 5. Adapter: apply_suggestion 同步 SelfModelManager
    # ============================================================

    def test_06_apply_suggestion_syncs_manager(self):
        """apply_suggestion 同步 SelfModelManager 理解指标"""
        from src.runtime.self_model.self_model_updater_adapter import SelfModelUpdaterAdapter

        store = SelfModelStore()
        updater = SelfModelUpdater(self_model_store=store)
        mgr = SelfModelManager("si_apply_test")
        adapter = SelfModelUpdaterAdapter(
            self_model_store=store,
            self_model_updater=updater,
            self_model_manager=mgr,
        )
        ...
        before_exp = mgr.identity.experience_awareness
        adapter.apply_suggestion(manager=mgr, suggestion=proposal)
        after_exp = mgr.identity.experience_awareness

        # 理解指标应有增长
        self.assertGreater(after_exp, before_exp)
        # version 应递增
        self.assertGreater(mgr.identity.version, 1)
    ...
    def test_01_end_to_end(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                # 3) Phase 4.0.3-C: 使用人工构造 proposal，确保 confidence >= 0.80
                #    以通过 GovernancePolicy trait 级别门槛（min_confidence=0.80）
