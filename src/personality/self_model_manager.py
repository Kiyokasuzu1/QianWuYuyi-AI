...
[ARCHITECTURE STATUS — Phase 4.0.1]
STATUS: COMPATIBILITY PROJECTION
  本模块是 RuntimeCore 内部的遗留/兼容人格状态模型。
  它不是当前 Production SelfModel Authority。
  当前 Authority 为: src/personality/self_model_store.py

  本模块的 SelfIdentity 数据结构与 SelfModelStore 的 SelfModel 是两套独立模型。
  未来 P4 架构迁移时，本模块可能降级或合并到 SelfModelStore 体系。

P4.2-IMPL-C4 更新:
- refresh() 新增 snapshot 参数（RelationshipSnapshot）
- 新增 _apply_relationship_snapshot()，从 snapshot.long_term 读取关系事实
- SelfModel 只读 long_term（一次互动不应改变自我认知）
...
class SelfModelManager:
    ...
    def refresh(
        ...
        *,
        ...
        P4.2-IMPL-C4: snapshot 优先于 relationship_state。
        """
        ...
        # 5. 关系状态 → 偏好 / 特质元信息
        # P4.2-IMPL-C4: 优先 snapshot（long_term），回退 legacy
        if snapshot is not None:
            self._apply_relationship_snapshot(snapshot)
    ...
    def load_full(self, data: Dict[str, Any]) -> None:
        ...
        # P5.0-C: 重建全部去重索引（含 growth_record_ids / insight_ids）
        self._rebuild_indexes()

    # ============================================================
    # P5.0-C: 从 Authority Store 持久化模型恢复长期自我认知
    # ============================================================

    def _rebuild_indexes(self) -> None:
        """基于当前 identity 内容重建去重索引。

        load_full() / restore_from_persisted() 共用；refresh() 增量方法依赖
        这些索引避免对已恢复条目重复合并。
        """
        self._growth_record_ids = set(
            e.source_id for e in self.identity.growth_history if getattr(e, "source_id", "")
        )
        self._insight_ids = set()
        for bp in self.identity.behavioral_patterns:
            self._insight_ids.update(bp.sources or [])
    ...
    def restore_from_persisted(self, model: Dict[str, Any]) -> bool:
        """
        P5.0-C: 从 Authority SelfModelStore 的持久化 SelfModel 恢复长期自我认知。

        恢复契约（详见 data/validation/p5_0_c_audit.md 第九节）：
        - 恢复：growth_narratives → growth_history / development_history
                self_understanding → 四个理解指标
                self_description.text → identity_understanding 前置条目
                identity_id → identity 元数据
        - 不恢复：current_traits / stable_traits / 情绪 / communication_style /
                  关系当前状态 / 瞬态上下文（本轮 refresh 重算或由专用模块负责）

        任何字段损坏都 fail-soft：跳过该字段，不抛异常。
        返回是否恢复到了至少一项内容。
        """
        ...
        # 1) identity_id（identity_name 在本结构无对应字段，跳过）
        iid = model.get("identity_id")
        if isinstance(iid, str) and iid.strip():
            self.identity.identity_id = iid
            restored_any = True

        # 2) growth_narratives → growth_history / development_history
        for gn in model.get("growth_narratives") or []:
            if not isinstance(gn, dict):
                continue
            rid = str(gn.get("record_id") or gn.get("source_growth_record_id") or "").strip()
            if not rid:
                continue
            # 幂等：已恢复（或已由 refresh 合并）过的记录跳过，避免重复追加
            if rid in self._growth_record_ids:
                continue
            try:
                conf = float(gn.get("_confidence", 0.5))
            except (TypeError, ValueError):
                conf = 0.5
            conf = max(0.0, min(1.0, conf))
            summary = str(gn.get("narrative") or gn.get("meaning") or "").strip()
            event = str(gn.get("event") or "").strip()
            dimension = str(gn.get("dimension") or "").strip()
            try:
                gh_entry = GrowthHistoryEntry(
                    source_type="growth_record",
                    source_id=rid,
                    affected_traits={},
                    summary=summary or "persisted growth narrative",
                    evidence_count=1,
                    confidence=conf,
                )
            except Exception:
                continue
            self.identity.growth_history.append(gh_entry)
            restored_any = True
            try:
                self.identity.development_history.append(
                    DevelopmentHistoryItem(
                        source_type="growth_record",
                        source_id=rid,
                        category=dimension or "growth",
                        what_changed=event or "self knowledge accumulated",
                        why_changed=summary or "persisted growth narrative",
                        evidence_count=1,
                        confidence=conf,
                        sources=[rid],
                    )
        ...
        # 3) self_understanding → 四个理解指标
        su = model.get("self_understanding")
        if isinstance(su, dict):
            def _bounded(key: str, default: float) -> float:
                try:
                    v = float(su.get(key, default))
                except (TypeError, ValueError):
                    v = float(default)
                return max(0.0, min(1.0, v))

            self.identity.experience_awareness = _bounded("experience_awareness", 0.0)
            self.identity.trait_awareness = _bounded("trait_awareness", 0.0)
            self.identity.identity_continuity = _bounded("identity_continuity", 0.0)
            self.identity.overall_understanding = _bounded("overall", 0.0)
            restored_any = True
        ...
        # 4) self_description.text → identity_understanding 前置条目
        #    注意：下一次 refresh() 会按实时信号重算 who_i_am，恢复的历史仍通过
        #    development_history 进入 what_changed / why_changed 持续消费。
        sd = model.get("self_description")
        desc_text = ""
        if isinstance(sd, dict):
            desc_text = str(sd.get("text") or "").strip()
        if desc_text:
            iu = self.identity.identity_understanding
            who = list(iu.who_i_am or [])
            item = f"self:{desc_text}"
            if item not in who:
                who.insert(0, item)
            iu.who_i_am = who[:8]
            restored_any = True
    ...
    # ============================================================
    # P5.0-D: 当前内存态 → Prompt 文本（Stage 14 消费）
    # ============================================================
    def format_context_for_prompt(self, max_narratives: int = 2) -> str:
        """把当前内存态渲染为【自我认知参考】文本（str）。

        结构与 SelfModelContextProvider.get_context() 的 legacy 文本一致：
        身份 / 活跃性格维度 / 信念 / 近期成长叙事；数据源为本 Manager 的
        当前内存态（get_full()），保证会话内最新写入（经 Store live-sync）
        在下一轮 Prompt 中立即可见，无需重启。

        任何字段损坏 / 渲染异常都 fail-soft 返回空字符串。
        """
        try:
            full = self.get_full()
            if not full:
                return ""
            lines = ["【自我认知参考】"]
            name = str(full.get("identity_id") or "").strip()
            if name:
                lines.append(f"身份：{name}")
            traits = full.get("stable_traits") or {}
            if isinstance(traits, dict):
                names = list(traits.keys())[:6]
                if names:
                    lines.append(f"当前活跃的性格维度：{', '.join(str(n) for n in names)}")
            values = full.get("core_values") or []
            if isinstance(values, list) and values:
                texts = []
                for v in values[:5]:
                    t = v.get("text") if isinstance(v, dict) else getattr(v, "text", None)
                    if t:
                        texts.append(str(t))
                if texts:
                    lines.append(f"你目前形成的信念：{'；'.join(texts)}")
            narratives = full.get("growth_history") or []
            if isinstance(narratives, list) and narratives:
                lines.append("近期重要的自我成长认知：")
                shown = 0
                for item in reversed(narratives):
                    text = item.get("summary") if isinstance(item, dict) else getattr(item, "summary", None)
                    if not text:
                        continue
                    text = str(text).strip()
                    if not text:
    ...
    def _apply_relationship_snapshot(self, snapshot: "RelationshipSnapshot") -> None:
        """
        P4.2-IMPL-C4: 从 RelationshipSnapshot.long_term 抽取关系相关偏好。

        只读，不修改外部。SelfModel 只读 long_term（一次互动不应改变自我认知）。
            ...
            if pref_key not in self._preference_keys:
                ...
                for p in self.identity.preferences:
                    if f"{p.domain}::{p.key}" == pref_key:
                        p.value = val
                        p.evidence_count += 1
                        if "relationship_snapshot" not in p.sources:
                            p.sources.append("relationship_snapshot")
                        break
