...
const YuyiRemote = (function () {
  ...
  /* ============ 管理 Token（安全修复：屏幕/控制端点需要鉴权） ============ */

  const TOKEN_STORAGE_KEY = "yuyi_admin_token";
  ...
  // 401 时提示输入管理 token 并重试一次；token 存 localStorage 后续自动携带
  async function _fetchWithAuth(url, options) {
    options = options || {};
    options.headers = _authHeaders(options.headers);
    let res = await fetch(url, options);
    if (res.status === 401) {
      const input = window.prompt("该操作需要管理 Token（YUYI_ADMIN_TOKEN），请输入：");
      if (input && input.trim()) {
        _setToken(input.trim());
        options.headers = _authHeaders(options.headers);
        res = await fetch(url, options);
        if (res.status === 401) {
          _setToken(""); // token 无效，清除避免反复使用错误值
        }
      }
    }
    return res;
  ...
  async function apiGet(path) {
    const res = await _fetchWithAuth(API_BASE + path, {});
