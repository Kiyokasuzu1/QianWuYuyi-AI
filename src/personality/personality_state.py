...
# P5.0-E: 初始基线（值与 R2.5.4 原默认完全一致；供「哪些维度真正被演化过」
# 的判定使用：apply_evolution 是 traits 的唯一修改入口，值偏离基线 ⟺ 被演化过）
DEFAULT_TRAIT_BASELINE: Dict[str, float] = {
...
class PersonalityState:
    ...
    """
    traits: Dict[str, float] = field(default_factory=lambda: dict(DEFAULT_TRAIT_BASELINE))
    version: int = 0
    ...
    def apply_evolution(self, record: EvolutionRecord) -> Dict[str, Any]:
        ...
        for trait_name, new_value in after_snap.items():
            ...
            # P5.0-E: 追加 "personality." 前缀剥离（Growth 提案 ChangeItem 使用该前缀）
            short_name = trait_key
            for strip_prefix in ("personality.", "trait.", "interests.", "interest."):
...
# ============================================================
# P5.0-E #4: 持久化辅助（任务卡要求 Personality 落盘；本模块冻结红线修订——
# 以新增辅助函数实现，不改 schema / 不改 apply_evolution 逻辑）
# ============================================================
DEFAULT_PERSONALITY_STATE_PATH = "data/personality_state.json"


def save_personality_state(
    state: Optional[PersonalityState] = None,
    path: Optional[str] = None,
) -> bool:
    """把当前（或指定）PersonalityState 序列化落盘。fail-soft：失败返回 False。"""
    try:
        st = state or get_personality_state()
        target = Path(path or DEFAULT_PERSONALITY_STATE_PATH)
        target.parent.mkdir(parents=True, exist_ok=True)
        # V1.0-OPT: 原子写（替换裸 open("w")+json.dump，消除截断窗口）
        from src.memory.atomic_write import atomic_write_json
        atomic_write_json(str(target), st.to_dict())
        return True
    except Exception as exc:  # noqa: BLE001
        logger.warning("personality_state 保存失败（已隔离）: %s", exc)
        return False


def load_personality_state(
    path: Optional[str] = None,
    *,
    into: Optional[PersonalityState] = None,
    current_version_ceiling: Optional[int] = None,
) -> bool:
    """从磁盘恢复全局（或指定）PersonalityState。

    - 版本单调保护：数据 version 低于 ceiling 时拒绝回退（保留现状）。
    - fail-soft：文件缺失/损坏/回退均返回 False，不抛异常。
    """
    try:
        target = Path(path or DEFAULT_PERSONALITY_STATE_PATH)
        ...
        with open(target, "r", encoding="utf-8") as f:
            data = json.load(f)
        restored = PersonalityState.from_dict(
            data,
            current_version_ceiling=current_version_ceiling,
        )
        if into is not None:
            into.traits = dict(restored.traits)
            into.version = restored.version
            into.evolution_record_ids = list(restored.evolution_record_ids)
            into._applied_proposal_ids = set(restored._applied_proposal_ids)
        else:
            global _global_state
            _global_state = restored
        return True
    except ValueError as exc:
        logger.warning("personality_state 恢复被拒绝（版本回退）: %s", exc)
        return False
    except Exception as exc:  # noqa: BLE001
        logger.warning("personality_state 加载失败（已隔离）: %s", exc)
        return False
