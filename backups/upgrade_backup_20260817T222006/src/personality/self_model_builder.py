"""
自我模型构建器 (SelfModelBuilder) v2.1.3 — DEPRECATED

Phase 3.8.5: 此 Builder 通过 SelfModelBuilderAdapter 桥接保留。
新代码请使用 self_model/self_model_builder.py 作为未来标准。
SelfModelStore 已通过 Adapter 接入，此文件保留用于向后兼容。

兼容：
- SelfModel v2 架构
- 旧测试接口
- SelfModelStore 调用方式（通过 Adapter）
