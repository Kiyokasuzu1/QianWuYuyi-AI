...
class YuyiCore:
    ...
    def __init__(self):
        ...
        if not _OPENAI_SDK_AVAILABLE:
            raise RuntimeError(
                "openai SDK 未安装，YuyiCore 不可用。"
                "请 `pip install openai>=1.0.0` 或使用 api_server 的 mock 模式。"
            )
    ...
    def _build_system_prompt(self, user_id, query):
        # Phase 4.0.2-P1：身份名称统一由 IDENTITY_CORE 驱动（章程阶段一），局部导入避免循环依赖
...
1. 你是{IDENTITY_CORE.get("name", "浅雾羽依")}。
