...
class SelfModelUpdaterAdapter:
    ...
    Phase 4.0.3-C: apply_suggestion() 安全闸 + _store fallback 闭合。所有 SelfModelStore 写入必须经过 Updater（含 GovernancePolicy），不再允许直接操作 _store。
    ...
    def apply_suggestion(
        ...
        **kwargs: Any,
        ...
        # 1) 通过 Updater.apply_proposal() 写入 Store
        # Phase 4.0.3-C (closure): 不再允许 _store fallback bypass。
        # 所有 SelfModelStore 写入必须经过 Updater（含 GovernancePolicy.evaluate()）。
        if self._updater is not None:
            ...
            logger.warning(
                "SelfModelUpdaterAdapter: _updater 为 None，"
                "拒绝写入 SelfModelStore（Governance bypass 已闭合）。"
            )
