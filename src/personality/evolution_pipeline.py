...
class EvolutionPipeline:
    ...
    def _execute_safe(
        ...
        # P5.0-E #4: 演化真实生效 → PersonalityState 立即落盘
        # （重启恢复依赖该文件；保存失败仅告警，不影响本次演化）
        if apply_result.get("applied"):
            from src.personality.personality_state import save_personality_state
            save_personality_state(self.state)
