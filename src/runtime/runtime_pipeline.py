...
class RuntimePipeline:
    ...
    def run(self, input_data: Any) -> "RuntimeContext":
        ...
        try:
            ...
            # Stage 2（记忆检索）与 Stage 14（回复生成 / PromptBuilder.history）。
            # 异常隔离：失败按无历史继续，不影响主链路。
            try:
                _orch_for_hist = getattr(self, "_orchestrator", None)
                if _orch_for_hist is not None and callable(
                    getattr(_orch_for_hist, "get_recent_history", None)
                ):
                    recent_history = _orch_for_hist.get_recent_history(
                        user_id=user_id, max_turns=20,
                    )
                    if isinstance(recent_history, list) and recent_history:
                        runtime_inputs["recent_history"] = recent_history
                        # V1.1.1-p2: 可观测性——每轮注入量落日志，"断片"问题一眼定位
                        logger.info(
                            "[RuntimePipeline] recent_history 注入 %d 条 (用户 %s)",
                            len(recent_history), user_id or "default",
                        )
            except Exception as exc_hist_inject:  # noqa: BLE001
                logger.warning(
                ...
                if runtime_ok:
                    ...
                    try:
                        self._orchestrator.record_conversation_turn(
                            ...
                            user_id=user_id,  # V1.1.1: 按用户隔离记录
                        )
