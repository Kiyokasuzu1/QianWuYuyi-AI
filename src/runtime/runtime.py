...
class RuntimeCore:
    ...
    def __init__(
        ...
        # ============================================================
        # P5.0-B: shell 基础生命周期状态已在上方无条件初始化完毕；
        # 本分支只负责创建 inner runtime 并建立委托关系，不再提前 return。
        # inner 创建失败 → _impl/_delegate 均为 None，shell 降级为合法自建对象。
        ...
        if _SHARED_MODE:
            ...
            # 旧 _delegate 语义保留（指向同一 impl；创建失败时同为 None，
            # 避免外部代码访问 _delegate 时 AttributeError）。
            self._delegate = impl
    ...
    def start(self) -> RuntimeContext:
        ...
        if delegated is not None:
            # P5.0-B: 委托成功时同步镜像 shell 启动状态，保持 shell 状态自洽
            self._started = True
            return delegated
