...
# ============================================================
# Admin 敏感端点鉴权（安全修复 2026-08-11）
#
# 背景：/admin/api/screen/* 可读取用户屏幕内容，
#       /admin/api/control/* 可远程操作鼠标键盘，
#       此前完全无鉴权；nginx 示例中的 IP 白名单默认处于注释状态。
#
# Token 解析顺序：
#   1. 环境变量 YUYI_ADMIN_TOKEN（推荐，不落盘、不入 git）
#   2. config.yaml 的 admin.token
#   3. 都未配置 → 仅允许本机回环访问（防 0.0.0.0 / LAN 暴露），启动时告警一次
#
# 客户端携带方式（二选一）：
...
def _get_admin_token() -> str:
    """解析管理 token：环境变量优先，其次 config.yaml admin.token。"""
    global _admin_token_cache
    token = os.getenv("YUYI_ADMIN_TOKEN", "").strip()
    if token:
        return token
    if _admin_token_cache is None:
        cfg = load_config()
        _admin_token_cache = str((cfg.get("admin") or {}).get("token", "") or "").strip()
    return _admin_token_cache


def _client_ip() -> str:
    """取真实客户端 IP；仅当直连方为本机回环（本地 nginx 反代）时才信任转发头。"""
...
def _is_loopback_client() -> bool:
    return _client_ip() in ("127.0.0.1", "::1", "localhost")


def _require_admin_auth(view_func):
    """Admin 敏感端点鉴权装饰器。

    策略（fail-closed）：
    - 已配置 token：必须携带匹配 token，否则 401
    - 未配置 token：仅放行本机回环，其余来源 403
    """
    ...
    def _wrapped(*args, **kwargs):
        ...
        if not token:
            if not _admin_token_warned:
                logger.warning(
                    "[安全] 未配置 YUYI_ADMIN_TOKEN，admin 屏幕/控制端点仅允许本机访问。"
                    "如需经 nginx/局域网访问管理面板，请配置该环境变量。"
                )
                _admin_token_warned = True
            if _is_loopback_client():
                return view_func(*args, **kwargs)
            return jsonify({
                "error": "管理端点未配置访问 token，仅允许本机访问",
                "hint": "设置环境变量 YUYI_ADMIN_TOKEN 后，携带 X-Admin-Token 请求头访问",
            }), 403
...
def _start_agent_server(config: dict):
    ...
    try:
        ...
        # 安全修复 2026-08-11：Agent Server token 解析改为 fail-closed
        # 背景（审计发现）：
        #   1. 此前只读 config.yaml 的 remote.auth_token，且仓库内硬编码了
        #      公开弱默认值 "test-token-yuyi-2026"
        #   2. .env.example 宣称支持 YUYI_REMOTE_TOKEN，但代码从未读取（文档说谎）
        #   3. src/remote/agent_server.py 的校验是 `if self.auth_token and ...`，
        #      token 为空时直接跳过鉴权 → 8765 端口对公网裸奔
        # 解析顺序：环境变量 YUYI_REMOTE_TOKEN → config remote.auth_token
        auth_token = os.getenv("YUYI_REMOTE_TOKEN", "").strip() or str(
            remote_cfg.get("auth_token", "") or ""
        ).strip()
        _WEAK_KNOWN_TOKENS = {
            "",
            "test-token-yuyi-2026",      # 仓库历史硬编码默认值
            "yuyi-agent-token-2026",     # local_agent 历史默认值
        ...
        if auth_token in _WEAK_KNOWN_TOKENS:
            logger.error(
                ...
                f"({auth_token!r})，在 {host}:{port} 上启动等于无鉴权暴露远程控制通道。"
                "修复方法：在服务器 .env 中设置 YUYI_REMOTE_TOKEN=$(openssl rand -hex 32)，"
                "并在 local_agent 端配置相同的值（YUYI_AGENT_TOKEN 或 local config token）。"
            )
            return
