...
class Orchestrator:
    ...
    def process(self, user_message: str, user_id: Optional[str] = None) -> str:
        ...
        Phase 7.2.1.1-identity-stabilization:
          UserResolver 从"身份决定者"降级为"身份缺失补全器"。
          - 只要入口 user_id 参数是有效值（非 None/非空），100% 信任，不调 resolver。
          - 只有 user_id 为 None 且 self.target_user_id 还没被设置过时，
            才 fallback 到 resolver.resolve()，保证极端旧链路不至于直接崩。
        """
        # ── 入口真相源：参数 user_id 是最高优先级 ──
        param_uid: Optional[str] = None
        if user_id is not None:
            s = str(user_id).strip()
            if s:
                param_uid = s
                self.target_user_id = s
        ...
        # Step 1: 解析用户身份（Phase 7.2.1.1：只在完全缺失时才 fallback resolver）
        user_context = None
        if param_uid is not None:
            # 参数可信 → 不调用 resolver；构造最小 UserContext 保持下游兼容性
            try:
                from src.identity.user_context import UserContext
                user_context = UserContext(user_id=param_uid, platform="qq")
            except Exception:  # noqa: BLE001
                user_context = None
            identity_source = "parameter"
        elif self.target_user_id:
            # 入口参数没传，但缓存里有（老代码路径）→ 不调 resolver
            identity_source = "cached_target_user_id"
        else:
            # 双重缺失 → 唯一允许使用 resolver 做身份补全
        ...
        # ── Phase 7.2.1.1：可观测日志，方便排查身份来源 ──
        try:
            logger.info(  # type: ignore[attr-defined]
                "[CognitiveContext] phase=OrchestratorLegacy identity_source=%s "
                "final_user_id=%s param_user_id=%s conversation_id=%s",
                identity_source,
                self.target_user_id or "NONE",
                param_uid or "NONE",
                conversation_id,
            )
        except Exception:  # noqa: BLE001
            # logger 不存在时静默通过（legacy 环境可能直接 print）
            pass
    ...
    def legacy_response(self, user_message: str) -> str:
        ...
        try:
            chat_memories = []
            try:
                chat_memories = self.memory_store.load() if self.memory_store else []
            except Exception:
                chat_memories = []
