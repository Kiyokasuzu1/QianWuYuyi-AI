...
# Phase 4.0.4-Pre: RuntimeContext 类型归一化。
# Pipeline/lifecycle 层可能把 frozen lifecycle_context.RuntimeContext 传入，
# 导致所有 ctx.xxx = yyy 赋值失败（cannot assign to field）。
# 此处保证：RuntimeCore 持有和操作的 ctx 永远是本模块的 mutable RuntimeContext。
# ── V1.1.1 Context Continuity ────────────────────────────────
# 记忆检索 query 融合最近 2-3 轮上下文（解决指代类消息检索无关键词问题）。
def _build_memory_query(
    user_message: str, history: Optional[List[Any]] = None,
) -> str:
    """构建记忆检索 query：最近 3 轮历史 + 当前消息（清洗 + 截头）。

    设计：
    - 当前消息先剥离宿主注入块（<RAG-Faiss-Memory> / <system_reminder>）
      再截头取 400 字符——避免"拿上一轮检索结果再当检索词"；
    - 前置最近 3 轮（6 条）历史文本，使"那那，羽依想做吗？"这类指代
      消息能带上先行词（做爱/文爱）参与语义检索；
    - 各段与总量封顶，保护 embedding 输入长度；history 为空时行为与
      旧实现一致（仅当前消息）。
    """
    try:
        from src.utils.text import strip_context_blocks
    except Exception:  # noqa: BLE001
        strip_context_blocks = lambda t: t if isinstance(t, str) else ""  # noqa: E731
    current = strip_context_blocks(user_message or "")[:400]
    if not history:
        return current
    parts: List[str] = []
    for item in history[-6:]:
        if isinstance(item, dict):
            ...
            if isinstance(content, str) and content.strip():
                role = item.get("role", "")
                prefix = "对方" if role == "user" else "羽依"
                parts.append(f"{prefix}:{strip_context_blocks(content)[:120]}")
    context_text = " ".join(parts)
    query = (context_text + " " + current).strip() if context_text else current
    return query[:1000]
...
# ============================================================
# P4.2-IMPL-C6B: ctx.relationship_snapshot 形态契约（Legacy Adapter 边界）
# ============================================================
# 输入规范（显式声明，禁止混用形态）：
#   ctx.relationship_snapshot 只承载 C.5 RelationshipRuntimeAdapter.read_relationship()
#   的 dict 输出（legacy 形态：relationship_available / stage / stage_label /
#   relationship_summary ...）。
#
# contracts.RelationshipSnapshot（current / long_term / provenance 对象契约）
# 不进入 ctx.relationship_snapshot，由 refresh_self_model_from_runtime(snapshot=...)
# 单独以对象形态传递并只经属性访问消费。
#
# 本函数是核心链路中唯一允许以 dict 形态读取 ctx.relationship_snapshot 的位置
# （由 tests/test_phase_4_2_c6b_snapshot_shape_contract.py AST 检查强制）。
# 传入非 dict 形态视为契约违约：告警并返回空（fail-soft），不做隐性兼容。


def _read_relationship_c5_fields(snap: Any) -> Dict[str, Any]:
    """从 C.5 legacy dict 快照中读取受限字段集（available/stage/stage_label/summary）。"""
    if snap is None:
        return {"available": False, "stage": "", "stage_label": "", "summary": ""}
    if not isinstance(snap, dict):
        logger.warning(
            "[RuntimeCore] ctx.relationship_snapshot 形态违约：期望 C.5 legacy dict，"
            "实际 %s（P4.2-IMPL-C6B 契约：contracts 对象不经由此字段流通）",
            type(snap).__name__,
        )
        return {"available": False, "stage": "", "stage_label": "", "summary": ""}
    if not snap.get("relationship_available"):
        return {"available": False, "stage": "", "stage_label": "", "summary": ""}
    return {
        "available": True,
        "stage": str(snap.get("stage") or "initial"),
        "stage_label": str(snap.get("stage_label") or ""),
        "summary": str(snap.get("relationship_summary") or ""),
    }


# V1.0: per-path 写锁（state_file 原子写 + 损坏备份保护，替换裸 open("w")）
_STATE_FILE_LOCKS: Dict[str, threading.RLock] = {}
_STATE_FILE_LOCKS_GUARD = threading.Lock()


def _state_file_lock(path) -> threading.RLock:
    """按绝对路径返回进程内 RLock（多实例同文件写串行化）。"""
    _key = str(path)
    with _STATE_FILE_LOCKS_GUARD:
        _lock = _STATE_FILE_LOCKS.get(_key)
        if _lock is None:
            _lock = threading.RLock()
            _STATE_FILE_LOCKS[_key] = _lock
    return _lock
...
class RuntimeCore(ModuleBase):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        if self.config.get("adapters_enabled", False):
            ...
            if memory_store_path:
                ...
                # V1.0-1B: 配置一致性保护 —— 自定义路径启用后该 store 即
                # 运行时权威实例；InteractionRecorder / events.handlers /
                # Orchestrator 已收口为 bridge-first 获取（Provider 仅 fallback），
                # 保证生产路径不会出现两个 authority。
                logger.info(
                    "RuntimeCore: 使用自定义 memory_store_path=%s"
                    "（运行时权威 store，获取方须经 RuntimeBridge）",
                    memory_store_path,
                )
                # V1.0-OPT: Memory Authority 分裂检测 —— 自定义路径下若
                # MemoryProvider 单例已被任何模块触及（默认路径实例存在），
                # 即出现两个 store 并存：读侧模块（event_extractor /
                # context_manager / topic_tracker 等）仍走 Provider 单例，
                # 写入侧走自定义路径 → 记忆分裂。fail-soft：仅报警不改行为。
                try:
                    from src.memory.memory_provider import MemoryProvider as _MP
                    _provider_instance = getattr(_MP, "_instance", None)
                    self._memory_authority_split_detected = (
                        _provider_instance is not None
                    )
                    if self._memory_authority_split_detected:
                        logger.warning(
                            "RuntimeCore: [Memory Authority 分裂检测] "
                            "memory_store_path=%s 与 MemoryProvider 单例（默认路径）并存。"
                            "读侧模块仍经 Provider 单例读取，存在记忆分裂风险；"
                            "请统一经 RuntimeBridge.get_memory_store() 获取权威 store。",
                            memory_store_path,
                        )
                except Exception as _mae:  # noqa: BLE001
                    logger.warning("RuntimeCore: Memory Authority 分裂检测失败（已隔离）: %s", _mae)
                    self._memory_authority_split_detected = False
            ...
            try:
                ...
                # P5.0-E #4: 运行时显式注入持久化 growth_history——
                # PersonalityResolver 裸构造默认是内存态（保持旧契约），
                # 运行时权威实例必须落盘（重启恢复 + SelfModel 重建的输入）。
                ...
                self.personality_resolver = PersonalityResolver(
                    ...
                    growth_history=PersonalityGrowthHistory(
                        storage_path=self.config.get(
                            "personality_growth_history_path",
                            "data/personality_growth_history.json",
                        ),
                    ),
                )
                # V1.0-1A: SelfModel Authority 收口 —— resolver 内部
                # self_model_store 裸构造默认为内存态（旧契约），运行时
                # 权威实例必须落盘：注入持久化 store 后，
                # get_self_model_store() / Orchestrator / SelfModelUpdater /
                # SelfModelContextProvider 全部经 resolver 共享同一写入者
                # （唯一 authority writer）。legacy 信封（YuyiPersistence
                # data 字段）兼容读取已在 SelfModelStore 内实现；
                # 注入失败仅告警并保持内存态（fail-soft，不阻断启动）。
                try:
                    from src.personality.self_model_store import SelfModelStore as _SMS
                    self.personality_resolver.self_model_store = _SMS(
                        storage_path=self.config.get(
                            "self_model_storage_path", "data/self_model.json"
                        )
                    )
                except Exception as _sms_e:
                    logger.warning(
                        f"SelfModelStore 持久化注入失败（保持内存态）: {_sms_e}"
                    )
            except Exception as _e:
            ...
            # P5.0-C: SelfModel 跨重启恢复 —— 启动期把 Authority SelfModelStore
            # 从磁盘恢复的长期自我认知灌进 SelfModelManager（Prompt 消费链来源），
            # 使"上一阶段形成的自我认知"在重启后继续影响后续生命周期。
            self._restore_self_model_authority()
        ...
        # P4.2-IMPL-C7: orchestrator 的 v0.6 long_term RelationshipState 引用
        # （由 OrchestratorRuntimeBridge 注入，见 _inject_engine_ref）。
        # 用于 _build_runtime_relationship_snapshot() 以 v0.6 为 long_term 权威；
        # 缺失时回退 C5 语义（long_term 由 relationship_state_runtime 推导）。
        self._orchestrator_long_term_state_ref: Any = None
    ...
    def _start(self) -> bool:
        ...
        try:
            ...
            # Phase 2.5-D: 关系核心治理接线(fail-soft,失败绝不影响启动)。
            # 锚点播种修复重启白名单丢失;桥接订阅使 runtime 独立运行时
            # 关系候选提取同样生效(与 Orchestrator 入口幂等共存)。
            try:
                self._seed_relationship_anchors()
            except Exception as e:  # noqa: BLE001
                logger.warning("关系核心锚点播种失败(降级): %s", e)
            try:
                self._ensure_relationship_candidate_bridge()
            except Exception as e:  # noqa: BLE001
                logger.warning("关系候选桥接订阅失败(降级): %s", e)
    ...
    def _load_state(self) -> None:
        ...
        如果文件不存在或损坏，使用默认状态。
        """
        # P5.0-E #4: 恢复持久化 PersonalityState（Growth 演化结果跨重启存活）。
        # 路径跟随 runtime state_file 所在目录（生产=data/），可经
        # config["personality_state_file"] 覆盖。版本单调保护；失败 fail-soft。
        try:
            from src.personality.personality_state import (
                get_personality_state,
                load_personality_state,
            )
            _ps_path = self.config.get("personality_state_file")
            if not _ps_path:
                _ps_path = str(self._state_file.parent / "personality_state.json")
            _ps = get_personality_state()
            load_personality_state(
                path=_ps_path,
                into=_ps,
                current_version_ceiling=int(getattr(_ps, "version", 0) or 0),
            )
        except Exception as exc:  # noqa: BLE001
            logger.warning("PersonalityState 恢复失败（已隔离）: %s", exc)

        # P0-1: 启动兜底——停机期间 admin 批准的 approved proposal 在此消费应用。
        # fail-soft：失败只记日志；人格状态与治理存储均不受影响。
        try:
            _startup_drain = self.drain_approved_growth_proposals()
            logger.info("[P0-1] 启动 growth drain 完成: %s", _startup_drain)
        except Exception as exc:  # noqa: BLE001
            logger.warning("[P0-1] 启动 growth drain 失败（已隔离）: %s", exc)
        ...
        try:
            ...
            # V1.0: 损坏备份——复制旧文件（不覆盖不删除），供人工恢复
            try:
                import shutil
                _backup = (
                    f"{self._state_file}.corrupt.{time.strftime('%Y%m%dT%H%M%S')}"
                )
                shutil.copy2(str(self._state_file), _backup)
                logger.warning(f"RuntimeCore: 损坏状态文件已备份为 {_backup}")
            except Exception:
                pass
    ...
    def _save_state(self) -> None:
        ...
        V1.0: atomic_write_json + per-path RLock（替换裸 open("w")+json.dump，
        消除截断窗口；数据格式不变）。
        """
        try:
            from src.memory.atomic_write import atomic_write_json
            self._state_file.parent.mkdir(parents=True, exist_ok=True)
            with _state_file_lock(self._state_file):
                atomic_write_json(
    ...
    def refresh_self_model(
        ...
        *,
        ...
        P4.2-IMPL-C5: snapshot（RelationshipSnapshot）优先于 relationship_state。
        """
    ...
    def refresh_self_model_from_runtime(self) -> Optional[Dict[str, Any]]:
        ...
        P4.2-IMPL-C5: 构建 RelationshipSnapshot 并传递给 refresh_self_model()。
        ...
        # P4.2-IMPL-C7: 构建 RelationshipSnapshot（统一入口，Stage 9 / Stage 14 共用）
        _snapshot = self._build_runtime_relationship_snapshot()
    ...
    def _build_runtime_relationship_snapshot(self) -> Any:
        """P4.2-IMPL-C7: RuntimeCore 统一的 RelationshipSnapshot 构建入口。

        数据源（不改 SnapshotBuilder 语义，不改两套 RelationshipState）：
        - current  ← relationship_state_runtime（v3.5.27 权威）
        - long_term ← 优先 _orchestrator_long_term_state_ref（v0.6 权威，
          由 OrchestratorRuntimeBridge 注入）；缺失时回退 C5 语义
          （long_term_state=relationship_state_runtime，由 builder 按
          v3.5.27 字段特征推导）。

        失败返回 None（调用方走 legacy fallback）。
        """
        try:
            from src.relationship.snapshot_builder import RelationshipSnapshotBuilder

            current_rs = getattr(self, "relationship_state_runtime", None)
            if current_rs is None:
                return None

            long_term_ref = getattr(
                self, "_orchestrator_long_term_state_ref", None
            )
            builder = RelationshipSnapshotBuilder()
            if long_term_ref is not None:
                return builder.build(
                    current_state=current_rs,
                    long_term_state=long_term_ref,
                    user_id=str(getattr(self, "_effective_user_id", "") or ""),
                )
            # Fallback: C5 既有语义（v3.5.27 → long_term 推导）
            return builder.build(long_term_state=current_rs)
        except Exception:
            return None
    ...
    def _forward_relationship_event_to_growth(
        ...
        try:
            ...
            if adapter is None:
                ...
                # Phase 2.4: 注入运行时权威 GrowthIntegrationService
                # （与 Stage 4 共用 growth_history / self_model_store），
                # 禁止适配器内部懒建独立 writer。装配失败返回 None 时
                # 适配器保持旧 fallback（fail-soft，不阻断关系主流程）。
                adapter = RelationshipEvidenceAdapter(
                    integration_service=self._get_growth_integration_service()
                )
    ...
    def get_self_model_store(self) -> Optional["SelfModelStore"]:
        ...
        # V1.0-OPT: 统一单实例 —— resolver store 是唯一 authority writer
        # （1A 注入时已按 config["self_model_storage_path"] 落盘，含隔离部署
        # override 场景）。仅当 resolver store 路径与 override 不一致
        # （注入失败 fail-soft 后的内存态）时，才回退 lazy store 读取覆盖路径。
        storage_override = self.config.get("self_model_storage_path")
        if self.personality_resolver is not None and hasattr(self.personality_resolver, "self_model_store"):
            _store = self.personality_resolver.self_model_store
            _store_path = getattr(_store, "_storage_path", None)
            _matches = (
                storage_override is None
                or _store_path is not None
                and Path(str(_store_path)).resolve() == Path(str(storage_override)).resolve()
            )
            if _matches:
                self._attach_live_sync(_store)
                return _store
            logger.warning(
                "RuntimeCore: resolver self_model_store 路径与 config 不一致"
                "（注入失败回退），使用 lazy store: %s vs %s",
                _store_path,
                storage_override,
            )
        ...
        try:
            ...
            if not hasattr(self, "_lazy_self_model_store") or self._lazy_self_model_store is None:
                ...
                # P5.0-C: 存储路径可经 config 覆盖（默认保持 data/self_model.json）
                storage_path = self.config.get(
                    "self_model_storage_path", "data/self_model.json"
                )
                self._lazy_self_model_store = _SelfModelStore(storage_path=storage_path)
            self._attach_live_sync(self._lazy_self_model_store)
            return self._lazy_self_model_store
        except Exception as _e:
            logger.warning(f"RuntimeCore: SelfModelStore lazy 创建失败: {_e}")
            return None

    def _attach_live_sync(self, store: Any) -> None:
        """P5.0-D: 把当前 SelfModelManager 绑定到权威 Store（会话内 live-sync）。

        - 绑定后，Store 每次持久化写入（update / apply_change_proposal / save）
          都会以内存方式同步 Manager 当前状态（store 侧回调，内存优先）
        - 幂等 / fail-soft：manager 缺失、store 不支持绑定或异常均静默跳过
        """
        manager = getattr(self, "self_model_manager", None)
        if store is None or manager is None:
            return
        try:
            if hasattr(store, "attach_self_model_manager"):
                store.attach_self_model_manager(manager)
        except Exception:  # noqa: BLE001
            pass

    def _restore_self_model_authority(self) -> None:
        """
        P5.0-C: 启动期把 Authority SelfModelStore 恢复的内容灌进 SelfModelManager。

        - store 由 get_self_model_store() 提供（lazy 创建即从磁盘读回持久化模型）
        - SelfModelManager 自身无持久化，本方法补上"重启后恢复"这一环
        - P5.0-D: 同时绑定 live-sync，使启动后的会话内写入也即时可见
        - 全部 fail-soft：任何异常只记 warning，不阻塞 Runtime 启动
        """
        try:
            manager = getattr(self, "self_model_manager", None)
            if manager is None or not hasattr(manager, "restore_from_persisted"):
                return
            store = self.get_self_model_store()
            if store is None:
                return
            self._attach_live_sync(store)
            model = store.get()
            if not isinstance(model, dict) or not model:
                logger.info("RuntimeCore: 无持久化 SelfModel 可恢复（首次启动或空存储）")
                return
            ok = manager.restore_from_persisted(model)
            logger.info(
                "RuntimeCore: SelfModelManager 已从权威 Store 恢复长期自我认知 (ok=%s, model_keys=%s)",
                ok, sorted(model.keys()),
            )
        except Exception as exc:  # noqa: BLE001
            logger.warning("RuntimeCore: SelfModel 权威恢复失败（已隔离）: %s", exc)
    ...
    def memory_authority_split_detected(self) -> bool:
        """V1.0-OPT: 启动期 Memory Authority 分裂检测结果。

        仅当 memory_store_path 自定义且 MemoryProvider 单例已被其他模块触及时
        为 True（存在双 store 风险）。检测失败/未启用时为 False（fail-soft）。
        """
        return bool(getattr(self, "_memory_authority_split_detected", False))
    ...
    def process(
        ...
        with self._process_lock:
            ...
            # 2) 初始化 ctx
            # Phase 4.0.4-Pre: ctx 归一化。外部可能传 frozen lifecycle_context，
            # 必须保证拿到的是本模块 mutable RuntimeContext，否则所有 Stage 赋值失败。
            ctx_was_foreign = (
                ctx is not None and not isinstance(ctx, RuntimeContext)
            )
            ctx = _normalize_runtime_ctx(ctx)
            if ctx_was_foreign:
                logger.info(
                    "[RuntimeCore.process] 归一化外部 RuntimeContext："
                    "original_type=%s → mutable RuntimeContext (session_id=%s)",
                    type(ctx).__module__ + "." + type(ctx).__name__
            ...
            try:
                ...
                # 若此处仍失败，说明 ctx 归一化未生效（仍为 immutable），
                # 打 WARNING 并立即构造新的 mutable ctx，避免 Stage 全失败。
                logger.warning(
                    "[RuntimeCore.process] ctx._phase_errors 写入失败，"
                    "强制重建 mutable RuntimeContext：type=%s, err=%s",
                    type(ctx).__name__, exc_ctx,
                )
                ctx = RuntimeContext()
                ctx._current_event = event  # type: ignore[attr-defined]
                ctx._phase_errors = self._last_process_phase_errors  # type: ignore[attr-defined]
    ...
    def _stage_02_memory_retrieval(
        ...
        if ma is not None and callable(getattr(ma, "retrieve", None)):
            try:
                # V1.1.1: query 融合最近 3 轮上下文（指代类消息可检索）
        ...
        if ms is not None:
            try:
                # Phase 2.5-B: scope 授权装配——owner 私人层 + YUI_CORE +
                # 其他对象的 global 关系核心(羽依自身约束,跨会话)。
    ...
    # --------------------------------------------------------
    # Phase 2.5-D: RelationshipCore 治理接线(只读注入 + 播种 + 桥接订阅)
    # --------------------------------------------------------
    def _get_relationship_core_store(self) -> Any:
        """Phase 2.5-D: 关系核心存储惰性单例(与 Orchestrator 同构,fail-soft)。"""
        try:
    ...
    def _seed_relationship_anchors(self) -> int:
        """Phase 2.5-D: 已审核关系核心的 anchor_memory_ids → 内存白名单。

        只读链路:RelationshipCoreStore → AnchorRegistry → CORE_RELATIONSHIP_MEMORY_IDS。
        任何失败返回 0(降级空集),绝不影响启动。
        """
        try:
    ...
    def _ensure_relationship_candidate_bridge(self) -> bool:
        """Phase 2.5-D: 幂等订阅关系候选桥接(事件消费者接线,fail-soft)。"""
        try:
    ...
    def _build_relationship_core_context_block(
        self, ctx: RuntimeContext, event: Optional[Event] = None,
    ) -> str:
        """Phase 2.5-D: 已审核关系核心 → ctx.relationship_core_context + Prompt 块。

        只读链路:RelationshipCoreStore → RelationshipCoreAdapter(按 visibility /
        relationship_id 过滤,其他用户拿不到清清的 relationship_only 核心)→
        build_relationship_context_block → [RELATIONSHIP_CONTEXT] 字符串。
        任何失败返回空串(不注入、不影响回复)。
        """
        try:
            ...
            if store is None:
                return ""
            uid = str(getattr(ctx, "user_id", None) or "").strip()
            if not uid:
                try:
                    uid = str(
                        self._extract_event_user_id(event, ctx)
                        or self.config.get("memory", {}).get("target_user_id")
                        or ""
                    )
                except Exception:  # noqa: BLE001
                    uid = ""
            if not getattr(ctx, "user_id", None):
                try:
                    ctx.user_id = uid  # type: ignore[attr-defined]
                except Exception:  # noqa: BLE001
                    pass
            try:
                RelationshipCoreAdapter(store=store, user_id=uid).process_cycle(ctx)
            except Exception:  # noqa: BLE001
                pass
            return build_relationship_context_block(store.load(), current_user_id=uid or None)
        except Exception:  # noqa: BLE001
            return ""
    ...
    def _stage_03_emotion_update(
        ...
        if em is None:
            getter = getattr(self, "get_emotion_manager", None)
            if callable(getter):
                try:
                    em = getter()
                except Exception:
        ...
        if em is not None:
            try:
                ...
                if callable(ufe) and event is not None:
                    ...
                    # P5.0-A 正式路径：复用 5B 降级链同一实现
                    # EmotionEventDetector.detect(text) → process_event →
                    # state.to_dict()（dict 契约：engine 消费 dominant/intensity，
                    # 关系更新/经历构建消费 mood/dominant）。
                    from src.emotion.emotion_event_detector import (
                        EmotionEventDetector,
                    )
                    # Stage 1 写入 ctx.user_message（payload.text/content）；
                    # user_input 为 legacy/单测兼容回退。
                    text = str(
                        getattr(ctx, "user_message", "")
                        or getattr(ctx, "user_input", "")
                        or ""
                    )
                    if text.strip():
    ...
    def _relationship_update(
        ...
        try:
            ...
            # 否则读到的关系数据是陌生人的（trust=0, familiarity=0），导致羽依不认识用户。
            event_user_id = self._extract_event_user_id(event, ctx)
            effective_rel_user_id = str(
                event_user_id
                or self.config.get("memory", {}).get("target_user_id")
                or self.config.get("user_id", "yuyi")
            )
            adapter = getattr(self, "_relationship_read_adapter", None)
            # user_id 变化时必须重建 adapter（不同用户不能复用缓存的关系数据）
            if adapter is None or getattr(adapter, "_effective_user_id", None) != effective_rel_user_id:
                ...
                adapter = RelationshipRuntimeAdapter(
                    ...
                    user_id=effective_rel_user_id,
                )
                adapter._effective_user_id = effective_rel_user_id  # type: ignore[attr-defined]
                adapter.attach()
                self._relationship_read_adapter = adapter
            # P4.2-IMPL-C6B 形态契约：ctx.relationship_snapshot 只承载 C.5 legacy dict
            # （读取仅限 _read_relationship_c5_fields 边界；contracts RelationshipSnapshot
            #  对象不经由此字段流通，见 refresh_self_model_from_runtime）
    ...
    def _build_relationship_prompt_context(
        ...
        """
        # P4.2-IMPL-C6B：读取必须经过唯一 Legacy Adapter 边界
        fields = _read_relationship_c5_fields(
            getattr(ctx, "relationship_snapshot", None)
        )
        if not fields["available"]:
            return {}
        out: Dict[str, Any] = {}
        stage_label = fields["stage_label"]
        if stage_label:
    ...
    def _build_communication_strategy_block(
        ...
        [RETAINED FOR COMPATIBILITY — P4.2-IMPL-C7 审计结论]
        本块与 RelationshipSnapshot facts 块（render_relationship_facts）
        不构成重复：此处只渲染解释深度/上下文引用/共同经历话题/情绪基调，
        不渲染 relationship_stage / trust / familiarity 档位；
        strategy dict 内虽携带 relationship_stage 字段，但从未进入本块文本。
        保留原样，待 C8 真实模型回归后再评估是否精简。
        """
        if not strategy:
            return ""
        try:
    ...
    def _get_growth_integration_service(self) -> Optional[Any]:
        ...
        try:
            ...
            # P5.0-E #5: 注入运行时权威实例——
            # growth_history（PersonalityResolver 持有）与 self_model_store（SelfModel Authority），
            # 让「演化 → Growth 记录 → SelfModel 刷新」在运行时唯一实例上生效。
            # 注意：resolver.growth_history 可能已被 _replay_trait_history 覆盖为只读
            # GrowthHistoryView；写成长记录必须用真实可写实例 _growth_history_store。
            _gh = getattr(self.personality_resolver, "_growth_history_store", None)
            if _gh is None:
                _gh = getattr(self.personality_resolver, "growth_history", None)
            service = GrowthIntegrationService(
                growth_history=_gh,
                self_model_store=self.get_self_model_store(),
                config=dict(self.config.get("growth_integration", {}) or {}),
            )
    ...
    def _stage_05_personality_update(
        ...
        if pr is not None and callable(getattr(pr, "resolve", None)):
            try:
                vector = pr.resolve()
                # P5.0-A: 此前 resolve() 的返回值被丢弃且立即 return，
                # ctx.personality_snapshot 永不写入（Stage 6 读空 → 人格不进 Prompt）。
                if vector is not None:
                    ctx.personality_snapshot = vector  # type: ignore[attr-defined]
                return
            except Exception as exc:
                logger.warning(
    ...
    def _stage_06_personality_context_build(
        ...
        """
        parts: List[str] = []
        # 人格快照（P5.0-A: PersonalityVector 无 to_dict / 无 traits 键，
        # 旧提取路径必然失败且 asdict 抛异常被吞 → personality_context_text 恒空。
        # 改为 get_all() 四层归一化；格式对齐 5B 降级链
        # Orchestrator._format_personality_vector，保留旧 traits 子 dict 分支）
        ps = getattr(ctx, "personality_snapshot", None)
        if ps is not None:
            try:
                if hasattr(ps, "get_all") and callable(getattr(ps, "get_all")):
                    pd_dict = ps.get_all() or {}
                elif hasattr(ps, "to_dict"):
                    pd_dict = ps.to_dict() or {}
                elif isinstance(ps, dict):
                    pd_dict = ps
                else:
                    pd_dict = asdict(ps)
                if isinstance(pd_dict, dict):
                    persona_summary = str(pd_dict.get("persona_summary") or "").strip()
                    if persona_summary:
                        parts.append("人格总结：" + persona_summary)
                    traits = pd_dict.get("traits") or {}
                    if traits:
                        top = sorted(
                            traits.items(),
                        ...
                        # 扁平向量：7 个核心表达维度定性档位（与 5B 链一致）
                        _CORE_FOR_PROMPT = (
                            ("warmth", "温暖"), ("gentleness", "温柔"),
                            ("shyness", "羞怯"), ("emotional_expression", "情绪外露"),
                            ("self_expression", "自我表达"), ("initiative", "主动性"),
                            ("care_level", "关心程度"),
                        )
                        trait_lines = []
                        for key, label in _CORE_FOR_PROMPT:
                            v = pd_dict.get(key)
                            if v is None:
                                continue
                            try:
                            ...
                            if vf >= 0.75:
                                ...
                                degree = "很低"
                            trait_lines.append(f"{label}={vf:.2f}（{degree}）")
                        if trait_lines:
                            parts.append("当前人格倾向：" + "，".join(trait_lines))
                    compact = str(pd_dict.get("compact_behavior") or "").strip()
                    if compact:
                        if len(compact) > 300:
                            compact = compact[:297] + "..."
                        parts.append("表达风格倾向：" + compact)
                    # P5.0-E #5: 演化后成长维度（resolver overlay 注入）。
                    # 只追加一行，不改变任何既有 section 顺序。
                    evolved = pd_dict.get("evolved_traits")
                    if isinstance(evolved, list) and evolved:
                        try:
                            ev_desc = ", ".join(
                                f"{e.get('trait')}={round(float(e.get('value') or 0), 2)}"
                                for e in evolved
                                if isinstance(e, dict) and e.get("trait")
                            )
                            if ev_desc:
                                parts.append("成长演化维度: " + ev_desc)
                        except Exception:
                            pass
            except Exception:
                pass
    ...
    def _stage_13_self_model_persistence(
        ...
        if getattr(ctx, "_control_blocked", False):
            return
        # P0-1: 消费 admin 已批准的 GrowthProposal（主循环唯一生产 apply 入口）。
        # fail-soft：任何异常只记日志，绝不影响聊天主流程。
        try:
            _drain_result = self.drain_approved_growth_proposals()
            self._sm_chain_mark(ctx, "growth_apply_drain", {
                "applied": _drain_result.get("applied", 0),
                "skipped": _drain_result.get("skipped", 0),
                "failed": _drain_result.get("failed", 0),
                "enabled": _drain_result.get("enabled", False),
            })
        except Exception as exc:  # noqa: BLE001
    ...
    def drain_approved_growth_proposals(
        self, limit: Optional[int] = None,
    ) -> Dict[str, Any]:
        """P0-1: 消费治理存储中 status=approved 的 GrowthProposal（唯一生产 apply 入口）。

        数据流：
          admin /growth/review → GovernanceProposal.status=approved（持久化队列）
              → 本方法（主循环 stage 13 + 启动兜底）
              → PersonalityEvolutionPipeline.apply_approved_to_state()
              → PersonalityState.apply_evolution + save_personality_state()

        约束：
          - 不直接修改 trait（唯一入口是 pipeline → PersonalityState.apply_evolution）
          - 单 proposal try/except：任何异常只记日志、保持 approved 供下轮重试
          - 落盘成功才标 applied；失败保持 approved（重启后自动重试）
          - 只消费 personality 类型；identity/relationship 提案跳过（由各自链路处理）
          - 不修改治理规则：approve 仍只由 admin 审核产生
        """
        enabled = bool(self.config.get("growth_apply_drain_enabled", True))
        try:
            _limit = int(limit if limit is not None else self.config.get("growth_apply_drain_limit", 5))
        except (TypeError, ValueError):
            _limit = 5
        result: Dict[str, Any] = {
            "enabled": enabled, "limit": _limit,
            "applied": 0, "skipped": 0, "failed": 0, "details": [],
        }
        if not enabled:
        ...
        if pipeline is None:
            result["reason"] = "pipeline_not_available"
            return result
        try:
            ...
            _ps = get_personality_state()
            _storage = get_proposal_storage()
            approved = _storage.list_by_status(PROPOSAL_STATUS["APPROVED"], limit=_limit)
        except Exception as exc:  # noqa: BLE001
            result["reason"] = f"storage_load_failed: {exc}"
            return result

        for legacy in approved:
            pid = str(getattr(legacy, "proposal_id", "") or "")
            entry: Dict[str, Any] = {"proposal_id": pid, "status": "skipped"}
            try:
                if _ps.has_proposal_been_applied(pid):
                    entry["reason"] = "already_applied"
                    result["skipped"] += 1
                    result["details"].append(entry)
                    continue
                if getattr(legacy, "proposal_type", "") != PROPOSAL_TYPE["PERSONALITY"]:
                    entry["reason"] = f"skip_type:{getattr(legacy, 'proposal_type', '')}"
                    result["skipped"] += 1
                    result["details"].append(entry)
                    continue
                # legacy governance proposal → canonical GrowthProposal（只读转换，不合并两套模型）
                _pcs = []
                _after_state = dict(getattr(legacy, "after_state", None) or {})
                _before_state = dict(getattr(legacy, "before_state", None) or {})
                for _trait, _after in _after_state.items():
                    try:
                        _after_f = float(_after)
                    except (TypeError, ValueError):
                        continue
                    try:
                        _before_f = float(_before_state.get(_trait, _after_f))
                    except (TypeError, ValueError):
                        _before_f = _after_f
                    _pcs.append(ChangeItem(
                        path=f"personality.traits.{_trait}",
                        before=_before_f,
                        after=_after_f,
                        reason=f"governance_approval:{getattr(legacy, 'reviewer_id', '') or 'admin'}",
                    ))
                if not _pcs:
                    entry["reason"] = "empty_after_state"
                    result["skipped"] += 1
                    result["details"].append(entry)
                    continue
                canonical = GrowthProposal(
                    id=pid,
                    proposed_changes=_pcs,
                    confidence=float(getattr(legacy, "confidence", 0.0) or 0.0),
                )
                canonical.status = "accepted"
                approval_id = (
                    f"{getattr(legacy, 'reviewer_id', '') or 'admin'}:"
                    f"{getattr(legacy, 'reviewed_at', '') or 'unknown'}"
                )
                ...
                if envelope.get("applied") and envelope.get("saved") is not False:
                    legacy.status = PROPOSAL_STATUS["APPLIED"]
                    legacy.applied_by = "runtime_drain"
                    legacy.applied_at = datetime.now(timezone.utc).isoformat()
                    _storage.save(legacy)
                    entry.update({
                        "status": "applied",
                        "saved": bool(envelope.get("saved")),
                        "record_id": envelope.get("record_id", ""),
                        "affected_traits": envelope.get("affected_traits", {}),
                    })
                    result["applied"] += 1
                else:
                    entry.update({
                        ...
                        "reason": envelope.get("reason") or (
                            "save_failed" if envelope.get("applied") else "apply_rejected"
                        ),
                    })
                    result["failed"] += 1
            except Exception as exc:  # noqa: BLE001
            ...
            result["details"].append(entry)
        logger.info(
            "[RuntimeCore.growth_drain] applied=%s skipped=%s failed=%s",
    ...
    def _stage_14_response_generation(
        ...
        # P4.2-IMPL-C7: 本 cycle 唯一一次 Snapshot 构建（与 Stage 9 refresh 共用入口，
        # 不在 ResponseAdapter 内重复构建）；facts 复用 C6C 正式渲染器。
        relationship_snapshot = self._build_runtime_relationship_snapshot()
        try:
            ctx.relationship_contracts_snapshot = relationship_snapshot  # type: ignore[attr-defined]
        except Exception:  # noqa: BLE001
            pass
        relationship_facts = ""
        try:
            from src.communication.relationship_facts import (
                render_relationship_facts,
            )
            relationship_facts = render_relationship_facts(relationship_snapshot)
        except Exception:  # noqa: BLE001
            relationship_facts = ""

        # Phase 2.5-D: [RELATIONSHIP_CONTEXT](已审核关系核心,只读 fail-soft)。
        # 注入两条回复路径:ResponseAdapter(context_prompt_blocks)与
        # orchestrator engine 降级路径(prompt_blocks);无核心/异常 → 空串。
        relationship_core_block = self._build_relationship_core_context_block(ctx, event)
        try:
            ctx.relationship_core_context_block = relationship_core_block  # type: ignore[attr-defined]
        except Exception:  # noqa: BLE001
            pass
        ...
        if adapter is not None and callable(getattr(adapter, "generate", None)):
            try:
                # P4.2-IMPL-C7: 构建真正的 ResponseRequest 对象（修复形态断裂——
                # 此前传裸 dict，ResponseAdapterImpl.generate 的属性访问必抛
                # AttributeError，导致 runtime adapter 路径每轮返回 fallback 文本）。
                # 局部 import 避免模块级循环依赖。
                from src.runtime.adapters.response_adapter import ResponseRequest

                # personality_context_text 是 str；ResponseRequest 契约为 dict
                pc_text = getattr(ctx, "personality_context_text", None)
                personality_context: Dict[str, Any] = (
                    {"personality_text": pc_text}
                    if isinstance(pc_text, str) and pc_text
                    else {}
                )
                # P5.0-A: 情绪快照随请求透传。emotion_state 保留原始 dict 快照
                # （审计/关系更新消费）；emotion_context 契约为 str
                # （ResponseAdapterImpl 按 str 透传至正式链 ResponseEngine，
                #  prompt_builder 原样追加为 section——故块头与结构在此生成，
                #  与 legacy engine.py dict 渲染的【当前情绪】块逐字一致）。
                emotion_snapshot = getattr(ctx, "emotion_snapshot", None)
                emotion_context_str: Optional[str] = None
                if isinstance(emotion_snapshot, dict):
                    dominant = (
                        emotion_snapshot.get("dominant")
                        or emotion_snapshot.get("mood")
                        or ""
                    )
                    if dominant:
                        try:
                            intensity = float(emotion_snapshot.get("intensity", 0))
                        except (TypeError, ValueError):
                            intensity = 0.0
                        emotion_context_str = (
                            f"【当前情绪】\n"
                ...
                # 文本原样透传（identity 沿用既有 strip 语义），过滤 None/空串。
                # identity 不再经 personality_context 注入，避免同一文本双通道重复。
                context_prompt_blocks: List[str] = []
                identity_text = getattr(ctx, "identity_context_text", None)
                if isinstance(identity_text, str) and identity_text.strip():
                    context_prompt_blocks.append(identity_text.strip())
                if strategy_block:
                    context_prompt_blocks.append(strategy_block)
                if behavior_guidance_block:
                    context_prompt_blocks.append(behavior_guidance_block)
                if relationship_core_block:
                    context_prompt_blocks.append(relationship_core_block)

                # user_id（Phase 4.0.4-Pre：对方是谁/怎么称呼）
                effective_user_id = ""
                try:
                ...
                # P5.0-D: SelfModel 文本（str 契约，impl 只透传 str）。
                # 用 Manager 当前内存态渲染——Stage 5 / Stage 12 的 live-sync
                # 已把本轮写入同步进来，因此同一轮回复即可消费最新自我认知。
                self_model_context_str: Optional[str] = None
                _sm_manager = getattr(self, "self_model_manager", None)
                if _sm_manager is not None and callable(
                    getattr(_sm_manager, "format_context_for_prompt", None)
                ):
                    try:
                        self_model_context_str = (
                            _sm_manager.format_context_for_prompt() or None
                        )
                    except Exception:  # noqa: BLE001
                        self_model_context_str = None

                req = ResponseRequest(
                    user_input=user_msg,
                    personality_context=personality_context,
                    # P4.2-IMPL-C7: Runtime 正式关系输入（resolver snapshot 路径 +
                    # facts 渲染的唯一数据源）
                    relationship_snapshot=relationship_snapshot,
                    relationship_context=(
                        relationship_facts if relationship_facts else None
                    ),
                    # Legacy fallback 数据：仅在 snapshot 缺失时被 impl 读取
                    relationship_state=self.get_relationship_state_snapshot(),
                    user_id=effective_user_id or None,
                    history=list(getattr(ctx, "history", []) or []),
                    chat_memories=list(getattr(ctx, "retrieved_memories", []) or []),
                    experience_context=list(experience_context or []),
                    # P5.0-D: 自我认知参考文本（优先路径此前完全缺失 SelfModel）
                    self_model_context=self_model_context_str,
                    # P5.0-A: 情绪快照透传（dict 原样 + compact str）
                    emotion_state=emotion_snapshot,
                    emotion_context=emotion_context_str,
                    # P4.4-D5: 三块已生成文本（identity/strategy/behavior）
                    # 随请求进入正式链；None 表示无内容（与旧行为一致）。
                    context_prompt_blocks=context_prompt_blocks or None,
                )
                reply = adapter.generate(req)
        ...
        if engine is not None and callable(getattr(engine, "generate", None)):
            try:
                ...
                reply = engine.generate(
                    ...
                    # P4.2-IMPL-C7: Snapshot facts（str 正式契约）；
                    # snapshot 不可用时回退 C.5 派生 dict（engine.py legacy k:v 分支）
                    relationship_context=(
                        relationship_facts
                        if relationship_facts
                        else self._build_relationship_prompt_context(ctx)
                    ),
    ...
    def _finish_chat_turn_experience(self, ctx: RuntimeContext) -> None:
        ...
        try:
            ...
            rel_stage = ""
            # P4.2-IMPL-C6B：读取必须经过唯一 Legacy Adapter 边界
            # （修复：此前读取的 key "relationship_stage" 在 C.5 输出中不存在，
            #  实际字段为 "stage"，导致 rel_stage 恒为空）
            rel_fields = _read_relationship_c5_fields(
                getattr(ctx, "relationship_snapshot", None)
            )
            if rel_fields["available"]:
                rel_stage = rel_fields["stage"]
