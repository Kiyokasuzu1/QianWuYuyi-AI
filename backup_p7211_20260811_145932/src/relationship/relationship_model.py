    ...
    def process_interaction(
        ...
        *,
        ...
        if evaluation and evaluation.passed and event is not None:
            trust_delta, familiarity_delta, collaboration_delta = self._plan_deltas(event.event_type)
            ...
            state.last_interaction_at = event.timestamp
            ...
            if state.trust != prev_trust:
                trust_change = RelationshipChange(
                    ...
                    confidence=event.signal_strength,
            ...
            if event.event_type == "trust_building" and state.trust >= 0.6:
                milestones.append(RelationshipMilestoneRecord(
            ...
            if event.event_type == "collaboration" and state.collaboration >= 0.5:
                milestones.append(RelationshipMilestoneRecord(
        ...
        record = RelationshipInteractionRecord(
            ...
            evidence_ids=list(event.evidence_ids) if event is not None else ([evidence_id] if evidence_id else []),
