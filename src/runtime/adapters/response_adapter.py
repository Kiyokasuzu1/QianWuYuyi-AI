...
# ============================================================
# Phase 4.0.4-Pre：user_meta 辅助函数（与业务解耦，失败返回 None/空）
# ============================================================


def _extract_ctx_user_id(ctx: Any) -> Optional[str]:
    """从 RuntimeContext 安全提取 user_id。找不到返回 None。"""
    if ctx is None:
        return None
    for attr in ("user_id", "target_user_id", "_user_id", "session_user_id"):
...
def _extract_relationship_profile(rel_state: Any) -> Optional[Dict[str, Any]]:
    """从 RelationshipState/任意对象安全抽取 relationship_profile dict。

    返回 dict（允许为空），找不到结构化信息返回 None。
    """
    ...
    if isinstance(rel_state, dict):
        for k_src, k_dst in (
            ...
            v = rel_state.get(k_src)
            if v is not None:
                profile[k_dst] = v
        return profile if profile else None
    # 2) 有 .to_dict() / .to_profile()
    for method in ("to_dict", "to_profile", "snapshot", "as_dict"):
        fn = getattr(rel_state, method, None)
        if callable(fn):
            try:
                d = fn()
            except Exception:
    ...
    for attr, dst in (
        ...
        v = getattr(rel_state, attr, None)
        if v is not None and (isinstance(v, str) or isinstance(v, int)):
            profile[dst] = v if isinstance(v, str) else str(v)
    return profile if profile else None
...
class ResponseRequest:
    ...
    # Phase 4.0.4-Pre：对方是谁（user_id）和关系画像（relationship_profile）。
    # 完全可选 / 空默认，向后兼容。ResponseAdapterImpl.generate() 将用这两个值
    # 调用 UserResolver.build_user_meta()，产出 preferred_name / calling_rule /
    # intimacy_rule 注入 Prompt。
    user_id: Optional[str] = None
    relationship_profile: Optional[Dict[str, Any]] = None
    # P4.2-IMPL-C7: Runtime 正式关系输入（contracts RelationshipSnapshot 对象）。
    # 类型用 Any 避免 runtime ↔ contracts 循环 import；运行时为
    # src.contracts.relationship_snapshot.RelationshipSnapshot（frozen dataclass，
    # 只经 .current / .long_term / .provenance 属性访问消费，见 C6B 契约）。
    # 正常路径下 ResponseAdapterImpl 只消费本字段；relationship_state 仅在
    # snapshot 缺失时作为 Legacy fallback 使用（两者分支互斥，禁止同时读取）。
    relationship_snapshot: Optional[Any] = None
    # P4.4-D5: 已生成系统提示块（identity_context / strategy_block /
    # behavior_guidance_block），由 RuntimeCore Stage 14 注入后原样透传至
    # ResponseEngine → PromptBuilder，每块作为独立 section 进入最终 system prompt。
    # None = 不注入（旧行为）；本层仅透传，不做任何文本加工。
    context_prompt_blocks: Optional[List[str]] = None
...
class ResponseAdapter(AdapterBase):
    ...
    def build_request(
        ...
        req = ResponseRequest(
            ...
            # Phase 4.0.4-Pre：对方是谁 + 关系画像。完全可选，向后兼容。
            user_id=_extract_ctx_user_id(ctx),
            relationship_profile=_extract_relationship_profile(relationship_state),
        )
