...
class GrowthIntegrationService:
    ...
    def __init__(
        ...
        self.growth_history = growth_history or PersonalityGrowthHistory(storage_path="data/personality_growth_history.json")
    ...
    def _record_applied_evolution(
        ...
        *,
        ...
        """演化真实生效（EvolutionPipeline executed=True）后写一条 PersonalityGrowthRecord。

        SelfModel 不自己猜 Growth：只有演化真正执行成功才产生本记录；
        SelfModel 只消费 PersonalityGrowthHistory。验证失败返回 None（fail-soft）。
        """
        changes: Dict[str, Any] = {}
        affected: List[str] = []
        for path, vals in (evolution_result.get("applied_traits") or {}).items():
            if not isinstance(vals, dict):
                continue
            try:
                before = float(vals.get("before") or 0.0)
                after = float(vals.get("after") or 0.0)
            except (TypeError, ValueError):
                continue
            short = str(path)
            for prefix in ("personality.", "trait."):
            ...
            changes[short] = {
                ...
                "reason": "approved_growth_evolution",
            }
        ...
        growth_level = str(em.get("growth_level") or "context")
        if growth_level not in {"context", "preference", "trait"}:
            growth_level = "context"

        narrative = str(em.get("narrative") or "")
        meaning_parts: List[str] = []
        for part in (
            narrative,
            str(em.get("reason") or ""),
            str(source_event.get("raw_content") or ""),
        ):
            part = part.strip()
            if part and part not in meaning_parts:
                meaning_parts.append(part)
        meaning = " / ".join(meaning_parts) if meaning_parts else (
            "成长演化已批准并应用: " + ", ".join(affected)
        )

        record = create_personality_growth_record(
            trigger_events=[str(source_event.get("id") or getattr(proposal, "id", "") or "")],
            changes=changes,
            ...
            source_growth_record_id=str(getattr(proposal, "id", "") or ""),
            evidence_ids=list(getattr(proposal, "evidence_ids", None) or []),
        )
        if self.growth_history.add(record):
    ...
    def accept_experience(
        ...
        try:
            ...
            # P5.0-E #1a/#1c: before 必须是 PersonalityState 当前值（不是 0.0），
            # after = clamp(before + delta)（applied_delta 是增量，不是绝对值）。
            # 读取失败 → 0.5 兜底，保证链路不中断。
            _ps_traits: Dict[str, float] = {}
            try:
                from src.personality.personality_state import get_personality_state

                _ps_traits = dict(get_personality_state().traits or {})
            except Exception:  # noqa: BLE001
                _ps_traits = {}
            ...
            if target_candidates and applied_delta > 0:
                for candidate in target_candidates:
                    ...
                    try:
                        _before = float(_ps_traits.get(candidate, 0.5) or 0.5)
                    except (TypeError, ValueError):
                        _before = 0.5
                    _after = round(min(max(_before + applied_delta, 0.0), 1.0), 6)
                    try:
                        ci = ChangeItem(path=path, before=_before, after=_after, reason=reason)
                    except TypeError:
                        ci = {
                            "path": path,
                            "before": _before,
            ...
            source_event: Dict[str, Any] = {
                # Phase 4.3-D：补 "id" 键（ProposalManager.create_proposal 读 "id"，
                # 此前只有 "event_id" 导致 proposal.source_event_id 恒 None）。
                "id": f"evt_{memory_id}",
                ...
                try:
                    ...
                    if _proposal_obj is not None:
                        ...
                        if _evo_res.get("executed"):
                            ...
                            # P5.0-E #3: 演化真实生效 → 写 PersonalityGrowthRecord（SelfModel 消费）
                            # 并立即刷新 SelfModel。失败仅告警，不阻塞主流程。
                            try:
                                _gr_id = self._record_applied_evolution(
                                    proposal=_proposal_obj,
                                    evolution_result=_evo_res,
                                    source_event=source_event,
                                )
                                if _gr_id:
                                    process_result["growth_record_id"] = (
                                        process_result.get("growth_record_id") or _gr_id
                                    )
                                self._refresh_self_model()
                            except Exception as _gr_exc:  # noqa: BLE001
                                logger.warning(
                                    "[accept_experience] 演化记录/SelfModel 刷新失败(已隔离): %s",
                                    _gr_exc,
                                )
