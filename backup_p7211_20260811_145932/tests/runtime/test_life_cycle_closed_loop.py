...
class TestLifeCycleClosedLoop(unittest.TestCase):
    ...
    def test_full_closed_loop(self):
        ...
        # Phase 7.2.1-p3: SelfState 默认 social_need 已改为 0.65；
        # 原测试语义依赖"默认 0.4 且收到用户消息后 0.4-0.1 ≈ 0.3"，
        # 这里在初始化后显式重置为旧值，不改动测试其他逻辑。
        core.self_state.social_need = 0.4
        core.self_state.initiative = 0.3
        core.world_state.self_state = core.self_state

        # 2. 注册自定义 send_message 处理器：补 message，再走 InitiativeBridge 的回调
        # Phase 7.2.1-p3：必须显式传入 target_user，否则 _resolve_target_user 返回 None
        # 时 InitiativeBridge 会打 ERROR 日志并直接 return（不再静默）
