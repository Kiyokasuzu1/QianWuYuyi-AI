...
const YuyiCognitive = (function () {
  ...
  async function fetchCognitive(endpoint) {
    try {
      ...
      // 失败时降级到 mock，保证 UI 仍有展示
      try {
        const fallback = await fetch(`${API_BASE}/${endpoint}?mock=true`);
...
async function handleProposal(id, action) {
  ...
  const actionEn = { accept: "approve", reject: "reject" }[action] || action;

  // 乐观更新 UI（先弹 Toast，再调 API）
  showToast(`正在${actionLabels[action] || action}成长建议 #${id}...`, "info");

  try {
    const res = await fetch(`/admin/api/cognitive/proposal/${id}/${actionEn}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ user_id: "admin", reason: "" }),
    });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json();

    if (data.success) {
      showToast(`${actionLabels[action] || action}成功：${data.message || ""}`, "success");
      if (typeof YuyiAudio !== "undefined") {
        YuyiAudio.play(action === "accept" ? "success" : "notification");
      }
      // 刷新认知视图，让方案列表更新
      if (typeof YuyiCognitive !== "undefined" && YuyiCognitive.loadAll) {
        setTimeout(() => YuyiCognitive.loadAll(), 500);
      }
    } else {
      showToast(`${actionLabels[action] || action}失败：${data.message || data.error || "未知错误"}`, "error");
    }
  } catch (e) {
    console.error("handleProposal failed:", e);
