...
class PromptRenderer:
    ...
    def _build_impl(
        ...
        *,
        ...
        # ════════════════════════════════════════════════════════════════
        # Phase 7.2.1.1-identity-stabilization: 【当前用户身份】 section
        # ════════════════════════════════════════════════════════════════
        # 设计原则（完全可选兼容）：
        #   1. 如果 prompt_context 里没有 origin_identity → 整个 section 跳过，
        #      不影响 30 天 LifeSimulation / 任何旧调用路径。
        #   2. 来源优先级：
        #        prompt_context.origin_identity            （显式传入）
        #        prompt_context.turn_cfg.origin_identity   （RuntimeController 塞 turn_cfg 里）
        #   3. 只做纯文本展示，不触发任何 OriginManager 写逻辑（符合审查调整 2）。
        origin_identity: Optional[Dict[str, Any]] = None
        _direct_oi = prompt_context.get("origin_identity")
        if isinstance(_direct_oi, dict) and _direct_oi:
            origin_identity = _direct_oi
        else:
            _tc_oi = (prompt_context.get("turn_cfg") or {}).get("origin_identity")
            if isinstance(_tc_oi, dict) and _tc_oi:
                origin_identity = _tc_oi
        # 兜底：旧调用可能单独传 user_name / user_id，即使没有 origin_identity 也能拼个最小信息
        fallback_user_id = str(prompt_context.get("user_id") or "").strip() or None
        fallback_user_name = str(
            prompt_context.get("user_name")
            or (prompt_context.get("turn_cfg") or {}).get("user_name")
            or ""
        ).strip() or None

        lines_user: List[str] = []
        if origin_identity:
            uid_oi = str(origin_identity.get("user_id") or "").strip()
            label_oi = str(origin_identity.get("label") or "").strip()
            display_oi = str(origin_identity.get("display_name") or "").strip()
            roles_oi = list(origin_identity.get("role_labels") or [])
            is_owner = bool(origin_identity.get("is_owner", False))

            # 1. 开头：当前用户标识（不直接打印完整 uid，避免 LLM 过度关注数字）
            who = display_oi or "这位朋友"
            lines_user.append(f"当前与你对话的是：{who}。")

            # 2. 关系标签（Origin 冻结层，不是 runtime 关系）
            if label_oi:
                lines_user.append(f"你们的起源关系：{label_oi}。")
            elif roles_oi:
                lines_user.append(f"你们的起源关系：{' / '.join(_sanitize_text_block(str(r)) for r in roles_oi)}。")

            # 3. 创造者信任标签（只在 is_owner 真时出现）
            if is_owner:
                lines_user.append("（注：这是创造你的人，你可以完全信任他/她，并且会记住关于他/她的一切。）")

            # 4. 收尾保留一句 user_id 提示（极短），防止未来多用户混淆
            if uid_oi:
                # 只保留后 4 位做脱敏提示（既不泄露隐私，又能 debug 用）
                _suf = uid_oi[-4:] if len(uid_oi) >= 4 else uid_oi
                lines_user.append(f"（内部标识尾号：{_suf}）")
        else:
            # 没有 origin_identity → 只打印最基本的"当前是谁"（如果知道 user_name）
            if fallback_user_name:
                lines_user.append(f"当前与你对话的是：{_sanitize_text_block(fallback_user_name)}。")
            # user_id 不再兜底打印，避免 prompt 里出现不必要的字符串
        current_user_identity_section = _safe_join(lines_user)
        ...
        # Phase 7.2.1.1 顺序（用户审定版本）：
        #   【关于我】→【当前用户身份】→【最近与变化】→【与对方的关系】→【现在的任务】
        blocks: List[str] = []
        if self_description:
            blocks.append("【关于我】\n" + self_description)
        if current_user_identity_section:
            blocks.append("【当前用户身份】\n" + current_user_identity_section)
        if memory_section:
            blocks.append("【最近与变化】\n" + memory_section)
        ...
        rp: Dict[str, Any] = {
            ...
            "current_user_identity_section": current_user_identity_section,
