...
class RuntimeController:
    ...
    def __init__(
        ...
        # Phase 7.2.1.1-identity-stabilization: OriginFacade（只读，懒加载）
        #   - 所有用户共享一个 Facade（因为 Origin 身份是全局冻结的）
        #   - 不在 __init__ 里实例化，避免 import 时不必要的依赖加载（测试可手动注入）
        self._origin_facade = None  # type: ignore[annotation-unchecked]
    ...
    def handle_message(
        ...
        *,
        ...
        with pm.acquire_lock():
            ...
            #   - Phase 7.2.2 才做强制阻断（SecurityError）；
            #   - 本阶段只打 WARNING 日志，避免升级后用户"突然失忆"。
            ...
            for r in memories:
                ...
                m_uid = r.get("user_id")
                if m_uid is None:
                    # 旧记忆没有 owner → Phase 7.2.1.1 放行但计数
                    missing_owner_count += 1
                    _filtered_memories.append(r)
                elif str(m_uid) == uid:
                    # 命中当前用户 → 放行
                    _filtered_memories.append(r)
                else:
                    # 跨用户记忆 → Phase 7.2.1.1 只过滤 + 计数（不抛错）
                    filtered_out_count += 1
            memories = _filtered_memories
            del _filtered_memories

            # [RUNTIME_IDENTITY] Phase 7.2.1.1 memory 水印审计日志（只打计数，不打内容）
            if missing_owner_count > 0 or filtered_out_count > 0:
                logger.warning(
                    "[RUNTIME_IDENTITY] memory_owner_audit | "
                    "user_id=%s | total=%d | missing_owner=%d | filtered_out_cross=%d | fallback_allowed=true | "
                    "建议 Phase 7.2.2 对 filtered_out>0 抛 SecurityError 并修复旧数据",
                    uid, total_before_filter, missing_owner_count, filtered_out_count,
                )
            ...
            # Phase 7.2.1.1-identity-stabilization: 懒加载 OriginFacade + 查询当前用户的 Origin 身份
            #   - 只做查询（不写、不判断关系升级），符合审查调整 2 要求
            if self._origin_facade is None:
                from src.identity.origin_facade import OriginFacade
                self._origin_facade = OriginFacade.default()
            origin_identity = self._origin_facade.get_user_identity(uid)

            # user_name 合并：入口显式 user_name 参数优先；否则用起源身份的 display_name（例如"清清"）
            effective_user_name = (
                user_name
                if (user_name and str(user_name).strip())
                else (origin_identity.get("display_name") or "")
            )
            ...
            turn_cfg: Dict[str, Any] = {
                ...
                # Phase 7.2.1.1-identity-stabilization: 起源身份信息（prompt_renderer 会读取）
                #   字典字段：user_id/is_owner/roles/role_labels/display_name/label
            ...
            if not reply.strip():
                ...
                return HandleResult(
                    ...
                    debug={
                        ...
                        # Phase 7.2.1.1-identity-stabilization（空回复时 origin 信息仍然保留在 debug 里方便排查）
                        "origin_label": origin_identity.get("label"),
                        "origin_is_owner": origin_identity.get("is_owner"),
                        "origin_display_name": origin_identity.get("display_name"),
                        "closeness_delta": 0.0,
                        "trust_delta": 0.0,
                    },
                )
        ...
        return HandleResult(
            ...
            debug={
                ...
                "companion_silent": health_signal.companion_silent,
                # Phase 7.2.1.1-identity-stabilization:
                #   - Origin 身份摘要（方便 debug=true 时验证 prompt 注入的身份是否正确）
                #   - Memory→Prompt 可观测性（memory_hits 表示进入 Prompt 的记忆条数）
                "origin_label": origin_identity.get("label"),
                "origin_is_owner": origin_identity.get("is_owner"),
                "origin_display_name": origin_identity.get("display_name"),
                "memory_context_count": len(cum_mems),  # 进入 Prompt 的实际 memory 条数
                # Phase 7.2.1.1 memory 水印审计计数（debug 时直接暴露，不打 content）
                "memory_owner_missing_count": missing_owner_count,
                "memory_cross_filtered_count": filtered_out_count,
            },
        )
