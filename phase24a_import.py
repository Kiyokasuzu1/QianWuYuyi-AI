# -*- coding: utf-8 -*-
"""Phase 2.4-A:T1 核心历史记忆受控导入脚本。

约束(严格遵守):
- 只通过现有 MemoryStore.add() 写入流程,不直接替换 memory.json
- 每条记录保留 metadata.recovery(完整 provenance)
- 不删除任何已有记录,不修改 identity/growth/emotion 任何文件
- 导入前在内存中先做 PollutionGuard 预检 + 内容指纹去重,跳过重复
- 全程只 import src.memory.*(不触碰 orchestrator/growth/emotion)

用法:
  python phase24a_import.py --payload phase24a/t1_payload.jsonl \
      --memory-path data/memory.json --report phase24a/import_report.json
  python phase24a_import.py --rollback phase24a/backup/memory.pre_import.json \
...
def build_memory(rec: Dict[str, Any]) -> Dict[str, Any]:
    """恢复记录 → MemoryStore schema(空值归零为 '',不伪造字段)。"""
    md = dict(rec.get("metadata") or {})
    md["source"] = "recovery_import_phase24a"
    return {
        "id": rec.get("id") or "",
...
def main() -> int:
    ...
    ap.add_argument("--payload", type=Path, default=Path("phase24a/t1_payload.jsonl"))
    ap.add_argument("--memory-path", type=Path, default=Path("data/memory.json"))
    ap.add_argument("--report", type=Path, default=Path("phase24a/import_report.json"))
