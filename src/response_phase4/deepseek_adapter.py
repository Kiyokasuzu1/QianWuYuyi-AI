...
def _fallback_system_content() -> str:
    """system 兜底文案 —— Phase 4.0.2-P1：由 IDENTITY_CORE 驱动（章程阶段一），
    替代原先硬编码的「你是羽依，一个会慢慢成长的 AI 伙伴。」。"""
    try:
        from src.personality.identity_core import IDENTITY_CORE
        return f"你是{IDENTITY_CORE.get('name', '浅雾羽依')}。{IDENTITY_CORE.get('essence', '')}"
    except Exception:  # noqa: BLE001
        # identity_core 不可用的极端兜底：保持最小中性描述，不引入新的身份来源
        return "你是浅雾羽依。"
...
class DeepSeekAdapter(LLMAdapterProtocol):
    ...
    def _build_messages(self, rendered_prompt: Dict[str, Any]) -> List[Dict[str, str]]:
        ...
        messages: List[Dict[str, str]] = [
            {"role": "system", "content": system_content or _fallback_system_content()},
