...
class OriginManager:
    ...
    def __init__(
        ...
        *,
        auto_freeze: bool = True,  # R2.7.6-YUYI: 生产默认 True（强制冻结），测试传 False 验证 persistence 机制
    ):
        ...
        # R2.7.6-YUYI: 如果加载的身份未冻结（旧版/空身份），自动升级为冻结身份
        #   auto_freeze=False（测试用）时跳过，允许 persistence 测试验证 save/load/evidence 流程
        if auto_freeze and not self._identity.is_frozen:
            frozen = OriginIdentity.create_frozen(CREATOR_USER_ID)
            # 保留旧 contributors 的 evidence（如果有），但冻结角色不接受新人
            for old_c in self._identity.contributors:
                if old_c.user_id == CREATOR_USER_ID:
                    # 清清的旧 evidence 合并到冻结身份
                    frozen_c = frozen.contributors[0]
                    for eid in old_c.evidence_ids:
                        if eid not in frozen_c.evidence_ids:
                            frozen_c.evidence_ids.append(eid)
            self._identity = frozen
